 ...
 *
 * <p>StreamGrabber asynchronously retrieves the output from an existing input stream in a thread,
 * and stores the output as an ArrayList of Strings. StreamGrabber is useful for retrieving the
 * output from multiple streams simultaneously. For example, StreamGrabber is used by
 * ExecUtils.ExecAndWait to retrieve standard output and standard error from an executed program.
 * You provide the existing input stream and character encoding of the input stream when creating
 * the StreamGrabber.
 */
public class StreamGrabber extends Thread {
  ...
  protected List<String> textLinesList = ListFactory.createNewList();
   ...
   *
   * @param inputStream The stream whose lines should be retrieved.
   */
  ...
  public void run() {
    ...
    try {
      ...
      UnicodeReader inputStreamReader = new UnicodeReader(inputStream, encoding);
