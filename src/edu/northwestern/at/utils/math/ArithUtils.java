 ...
 *
 * <p>The following methods for trigonometric functions come from the Sfun class written by Visual
 * Numerics.
 *
 * <ul>
 *   <li>acosh
 ...
 * </ul>
 *
 * <p>These methods are covered by the following license.
 * ------------------------------------------------------------------------- $Id: Sfun.java,v
 * 1.1.1.1 1999/03/05 21:43:39 brophy Exp $
 * ------------------------------------------------------------------------- Copyright (c) 1997 -
 * 1998 by Visual Numerics, Inc. All rights reserved.
 *
 * <p>Permission to use, copy, modify, and distribute this software is freely granted by Visual
 * Numerics, Inc., provided that the copyright notice above and the following warranty disclaimer
 * are preserved in human readable form.
 *
 * <p>Because this software is licensed free of charge, it is provided "AS IS", with NO WARRANTY. TO
 * THE EXTENT PERMITTED BY LAW, VNI DISCLAIMS ALL WARRANTIES, EXPRESS OR IMPLIED, INCLUDING BUT NOT
 * LIMITED TO ITS PERFORMANCE, MERCHANTABILITY AND FITNESS FOR A PARTICULAR PURPOSE. VNI WILL NOT BE
 * LIABLE FOR ANY DAMAGES WHATSOEVER ARISING OUT OF THE USE OF OR INABILITY TO USE THIS SOFTWARE,
 * INCLUDING BUT NOT LIMITED TO DIRECT, INDIRECT, SPECIAL, CONSEQUENTIAL, PUNITIVE, AND EXEMPLARY
...
public class ArithUtils {
   ...
   *     <p>If x is NaN or less than one, the result is NaN.
   ...
   * k".
   *
   * <p>The binomial coefficient is defined as <tt>(n * n-1 * ... * n-k+1 ) / ( 1 * 2 * ... * k
   * )</tt>.
   *
   * <ul>
   *   <li>k<0<tt>: <tt>0</tt>.
   ...
   * @return exp( x ) - 1.
   *     <p>Example: expm1( 9.995003330835334E-4 ) is 0.001 .
   *     <p>Implements a method suggested by William Kahan.
   */
   ...
   *     <p>This is an implementation of an algorithm suggested by Donald E. Knuth in Section 4.2.2
   *     of <em>Seminumerical Algorithms (3rd edition)</em>.
   */
   ...
   *     <p>The hypotenuse value is given mathematically as the sqrt( a^2 + b^2 ). The method
   *     implemented here reduces the chances of cancellation and roundoff error. If the |a| > |b|,
   *     we compute the hypotenuse as:
   *     <p>hypotenuse = |a| * sqrt( 1 + (b/a) * (b/a) )
   *     <p>Otherwise b != 0 compute the hypotenuse as:
   *     <p>hypotenuse = |b| * sqrt( 1 + (a/b) * (a/b) )
   *     <p>If b is zero, the hypotenuse is zero.
   */
   ...
   *     <p>Example: log10( 100.0D ) is 2.0D .
   */
   ...
   *     <p>Example: log1p( 0.001D ) is 9.995003330835334E-4
   *     <p>Implements a method suggested by William Kahan.
   */
   ...
   *     <p>Example: trunc( 30.12345D ) is 30.0D .
   */
