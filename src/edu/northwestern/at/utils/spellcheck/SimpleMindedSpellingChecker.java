 ...
 *
 * <p>Implements a very simple minded spelling checker using HashMaps to hold the dictionaries. The
 * dictionaries are read as combined word lists and metaphone codes from text files into two
 * hashmaps. One hashmap contains each word as a key and has the metaphone code for that word as a
 * value. The second hashmap uses the metaphone values as keys and the list of words mapping to that
 * metaphone as values. This allows presentation of suggested spellings for misspelled words. The
 * list of suggestions may optionally be pruned by using a measure of the Levenstein distance
 * between the original misspelling and each suggested spelling.
 *
 * <p>Words beginning with a digit are assumed to be numbers and therefore spelled correctly.
 *
 * <p>When creating a list of suggested replacements for a misspelled word, the first letter of the
 * suggestions is capitalized if the first letter of the original misspelled word is capitalized.
 *
 * <p>This class is not intended for production use because of the time needed to load and save
 * dictionaries and the amount of memory used to hold the dictionaries in memory. This class serves
 * as a reference implementation for the SpellingChecker interface as well as a testbed for
 * applications needing a spelling checker during development.
 */
public class SimpleMindedSpellingChecker implements SpellingChecker {
  ...
  public SimpleMindedSpellingChecker(
      SpellingDictionary globalDictionary, SpellingDictionary localDictionary) {
   ...
   *
   * @param dictionary Identifies dictionary. The dictionary class must implement the
   *     SpellingDictionary interface.
   *     <p>The global dictionary is usually shared among many users. Typically it is created by a
   *     system administrator as a shareable resource.
   */
   ...
   *
   * @param dictionary The dictionary to use. If the name is null, no local dictionary is used. The
   *     dictionary class must implement the SpellingDictionary interface.
   *     <p>The local dictionary is usually limited to access by a single individual. The local
   *     dictionary generally contains words added by an individual while checking the spelling of
   ...
   * @return True if the word was found in the global or local dictionaries.
   */
  public boolean checkSpelling(String word) {
    ...
    if (Character.isDigit(word.charAt(0))) return true;
    ...
    return ignoreList.lookupWord(lowerCaseWord)
        || ((localDictionary == null) ? false : localDictionary.lookupWord(lowerCaseWord))
        || ((globalDictionary == null) ? false : globalDictionary.lookupWord(lowerCaseWord));
  }
   ...
   * @return Possibly pruned String set of suggested words. The suggestions are pruned using the
   *     Levenstein distance.
   */
  private Set<String> pruneSuggestions(String word, Collection<String> suggestions) {
    ...
    while (iterator.hasNext()) {
      ...
      if (LevensteinDistance.areAlike(word, suggestedWord)) result.add(suggestedWord);
    }
   ...
   *
   * @param word The misspelled word for which possible alternatives are desired.
   * @param prune True to prune suggestions using Levenstein distance.
   * @return Sorted array of correctly spelled words which are similar in some sense to the
   *     misspelled word.
   *     <p>The suggested words are typically words which are similar in sound or spelling to the
   *     misspelled word. If the misspelled word begins with a capital letter, the suggestions are
   *     also capitalized to match.
   */
  public String[] suggest(String word, boolean prune) {
    ...
    try {
      ...
      if (globalDictionary != null)
        addWords(combinedSuggestions, globalDictionary.getRelatedWords(metaphoneValue));

      if (localDictionary != null)
        addWords(combinedSuggestions, localDictionary.getRelatedWords(metaphoneValue));

      addWords(combinedSuggestions, ignoreList.getRelatedWords(metaphoneValue));

      if (!metaphoneValue2.equals(metaphoneValue)) {
        if (globalDictionary != null)
          addWords(combinedSuggestions, globalDictionary.getRelatedWords(metaphoneValue2));

        if (localDictionary != null)
          addWords(combinedSuggestions, localDictionary.getRelatedWords(metaphoneValue2));

        addWords(combinedSuggestions, ignoreList.getRelatedWords(metaphoneValue2));
      }
      ...
      if (suggestions != null) {
        ...
        if (prune) {
          suggestions = pruneSuggestions(lowerCaseWord, suggestions);
        }
   ...
   * @return True if word added successfully.
   *     <p>Typically this function is retricted to system administrators.
   */
   ...
   *     <p>The ignore list contains words which an individual marks as ignorable while checking the
   *     spelling of one of more documents. The ignore list is transient. If an ignored word is to
   *     persist across multiple spelling check sessions, the word should be added to the local
   *     dictionary.
   */
   ...
   *
   * <p>Removes all words from the ignore list. Typically this is invoked at the start of a spelling
   * checker session for a new document.
   */
