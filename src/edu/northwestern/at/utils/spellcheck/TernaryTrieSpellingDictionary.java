 ...
 *
 * <p>This class provides the basic methods for a spelling checker dictionary implemented using a
 * ternary trie.
 */
public class TernaryTrieSpellingDictionary implements SpellingDictionary {
   ...
   *
   * @param wordsList Tagged strings list with words to add to the dictionary.
   */
   ...
   * @return Set of words which are similar to specified word.
   */
  ...
  public Set<String> findMostSimilarSet(String word) {
    ...
    for (maxSuggestions = 1;
        (maxSuggestions <= MAXDIFFS) && (size >= maxSuggestions * 2);
        maxSuggestions++) {
      result.addAll(trie.nearSearch(word, maxSuggestions));
    }
    ...
    if ((result.size() == 0) && (size > 2)) {
      for (maxSuggestions = 0; (maxSuggestions <= 1); maxSuggestions++) {
        for (i = size - 1; i >= 0; i--) {
          result.addAll(
              trie.nearSearch(word.substring(0, i) + word.substring(i + 1, size), maxSuggestions));

          for (j = 'a'; j < 'z'; j++) {
            result.addAll(
                trie.nearSearch(
                    word.substring(0, i) + ((char) j) + word.substring(i, size), maxSuggestions));
          }
        }
      }
    }
  ...
  public Set<String> getMoreRelatedWords(String word) {
    ...
    if ((word != null) && (word.length() > 0)) {
      ...
      for (int i = (MAXDIFFS + 1); i < Math.min(l / 2, 10); i++) {
        List<String> nearSearches = trie.nearSearch(word.toLowerCase(), i);
