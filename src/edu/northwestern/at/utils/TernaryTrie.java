...
/**
 * <code>TernaryTrie</code> is an implementation of a ternary tree. Methods are provided for
 * inserting strings and searching for strings. The algorithms in this class are all recursive, and
 * have not been optimized for any particular purpose. Data which is inserted is not sorted before
 * insertion, however data can be inserted beginning with the median of the supplied data.
 *
 * @author <a href="mailto:dfisher@vt.edu">Daniel Fisher</a>
 * @version $Revision: 2134 $ $Date: 2005-03-28 14:48:04 -0500 (Mon, 28 Mar 2005) $
 */
public class TernaryTrie implements TaggedStrings {
   ...
   *
   * @param stringsMap The map whose keys and values are to be added to the trie.
   * @param addValuesAsKeys True to add each value as a key as well.
   */
   ...
   *
   * @param stringsSet The set whose values are to be added to the trie.
   */
   ...
   *
   * @param stringsList The list whose values are to be added to the trie.
   */
   ...
   *
   * @param stringsList The tagged strings list whose values are to be added to the trie.
   * @param addValuesAsKeys True to add each value as a key as well.
   */
  public TernaryTrie(TaggedStrings stringsList, boolean addValuesAsKeys) {
    Iterator<String> iterator = stringsList.getAllStrings().iterator();
   ...
   *
   * @param collection Collection whose values are to be inserted into trie.
   */
  ...
  /**
   * This will return an array of strings which partially match the supplied word. word should be of
   * the format '.e.e.e' Where the '.' character represents any valid character. Possible results
   * from this query include: Helene, delete, or severe Note that no substring matching occurs,
   * results only include strings of the same length. If the supplied word does not contain the '.'
   * character, then a regular search is preformed.
   *
  ...
  public List<String> partialSearch(String word) {
    ...
    return partialSearchNode(root, list, "", word, 0);
  }
  ...
  public List<String> prefixSearch(String prefix) {
    ...
    if (prefix != null) {
      String prefixPattern = prefix + StringUtils.dupl(".", maxKeyLength - prefix.length());

      List<String> list = ListFactory.createNewList();

      matches = prefixSearchNode(root, list, "", prefix, prefixPattern, 0);
    }
  ...
  protected List<String> prefixSearchNode(
    ...
    if ((node != null) && (index < prefixPattern.length())) {
      ...
      if ((c == '.') || (c < split)) {
        matches = prefixSearchNode(node.getLokid(), matches, match, prefix, prefixPattern, index);
      }
      ...
      if ((c == '.') || (c == split)) {
        ...
        matches =
            prefixSearchNode(
                node.getEqkid(), matches, partialWord, prefix, prefixPattern, index + 1);
      }

      if ((c == '.') || (c > split)) {
        matches = prefixSearchNode(node.getHikid(), matches, match, prefix, prefixPattern, index);
      }
    }
  ...
  /**
   * This will return an array of strings which are near to the supplied word by the supplied
   * distance. For the query nearSearch("fisher", 2): Possible results include: cipher, either,
   * fishery, kosher, sister. If the supplied distance is not > 0, then a regular search is
   * preformed.
   *
   * @param word <code>String</code> to search for
  ...
  public List<String> nearSearch(String word, int distance) {
    ...
    return nearSearchNode(root, distance, list, "", word, 0);
  }

  /**
   * This will return an array of all the words in this <code>TernaryTrie</code>. This is a very
   * expensive operation, every node in the tree is traversed.
   *
  ...
  /**
   * This will recursively insert a word into the <code>TernaryTrie</code> one node at a time
  ...
  protected TernaryTrieNode insertNode(
      TernaryTrieNode node, final String word, final int index, final Object value) {
  ...
  /**
   * This will recursively search for a word in the <code>TernaryTrie</code> one node at a time
   * beginning at the supplied node.
   *
   ...
   */
  protected boolean searchNode(final TernaryTrieNode node, final String word, final int index) {
  ...
  protected TernaryTrieNode findNode(
      final TernaryTrieNode node, final String word, final int index) {
    if (node != null && index < word.length()) {
      final char c = word.charAt(index);
      final char split = node.getSplitChar();
  ...
  /**
   * This will recursively search for a partial word in the <code>TernaryTrie</code> one node at a
  ...
  /**
   * This will recursively search for a near match word in the <code>TernaryTrie</code> one node at
  ...
  /**
   * This will recursively traverse every node in the <code>TernaryTrie</code> one node at a time
   * beginning at the supplied node. The result is a string representing every word, which is
   * delimited by the LINE_SEPARATOR character.
   *
   * @param node <code>TernaryTrieNode</code> to begin traversing
   * @param s <code>String</code> of words found at the supplied node
   * @param words <code>ArrayList</code> which will be returned (recursive function)
