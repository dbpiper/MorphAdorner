 ...
 *     <p>Modications by Philip R. "Pib" Burns to add missing BitSet methods.
 */
public class SparseBitSet implements XCloneable, Serializable {
  ...
  protected static final BinOp OR =
      new BinOp() {
        public final long op(long a, long b) {
          return a | b;
        }
  ...
  protected static final BinOp XOR =
      new BinOp() {
        public final long op(long a, long b) {
          return a ^ b;
        }
  ...
  protected static final BinOp ANDNOT =
      new BinOp() {
        public final long op(long a, long b) {
          return a & ~b;
        }
   ...
   * @throws IndexOutOfBoundsException if specified index is negative.
   *     <p>If no such bit exists then -1 is returned. To iterate over the true bits in a BitSet,
   ...
   * @throws IndexOutOfBoundsException if specified index is negative.
   *     <p>If no such bit exists then -1 is returned. To iterate over the false bits in a BitSet,
   *     use the following loop:
   *     <p><code>
  ...
  /**
   * Return an <code>Enumeration</code> of <code>Integer</code>s indices of bits set on in this
   * SparseBitSet.
   */
