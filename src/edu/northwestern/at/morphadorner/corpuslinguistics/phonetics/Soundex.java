 ...
 *
 * <p>Soundex hashes words to a smaller space using a simple model which approximates the sound of
 * the word as produced by a native American English speaker. The hash is a four (usually) character
 * string in which the first character is an uppercase letter and the remaining characters are
 * digits. Soundex was originally intended only for encoding proper last names, but occasionally
 * finds other uses as well. The Soundex algorithm was devised and patented by Margaret K. Odell and
...
public class Soundex {
   ...
   * @return The Soundex code for "s". Returns an empty string when the Soundex code cannot be
   *     found. In particular, a Soundex code cannot be found if the first character in "s" is not a
   *     letter (a-z, A-Z).
   */
  public static String soundex(String s) {
    ...
    for (int i = 1; (i < sUpperCase.length()) && (result.length() < MAXSOUNDEXLENGTH); i++) {
