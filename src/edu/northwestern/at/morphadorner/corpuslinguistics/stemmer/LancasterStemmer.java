 ...
 *
 * <p>Paice/Husk Stemmer - License Statement.
 *
 * <p>This software was designed and developed at Lancaster University, Lancaster, UK, under the
 * supervision of Dr Chris Paice. It is fully in the public domain, and may be used or adapted by
 * any organisation or individual. Neither Dr Paice nor Lancaster University accepts any
 * responsibility whatsoever for its use by other parties, and makes no guarantees, expressed or
 * implied, about its quality, reliability, or any other characteristic.
 *
 * <p>It is assumed that, as a matter of professional courtesy, anyone who incorporates this
 * software into a system of their own, whether for commercial or research purposes, will
 * acknowledge the source of the code.
 *
 * <p>Modified from the original Java programs written by Christopher O'Neill and Rob Hooper.
 */
public class LancasterStemmer implements Stemmer {
  /** Prefixes to remove from words before stemming. */
  public static final String[] prefixes = {
    "intra", "kilo", "mega", "micro", "milli", "nano", "pico", "pseudo", "ultra",
  };

  /**
   * Default stemming rules.
   *
   * <p>These rules MUST be stored in ascending alphanumeric order of the first character.
   */
   ...
   *     <p>Prefixes are automatically removed from words with more than two characters.
   */
   ...
   *
   * @param rules The stemming rules as an array of String.
   * @param preStrip True to remove prefixes from words with more than two characters.
   *     <p>Prefixes are automatically removed from words with more than two characters.
   */
  ...
  protected int firstVowel(String s, int last) {
    ...
    for (i = 0; (i < last) && (!(vowel(s.charAt(i), prevChar))); i++) {
      prevChar = s.charAt(i);
    }
  ...
  protected String stripSuffixes(String s) {
    ...
    while (((lastLetterPos + 1) < stem.length()) && isLetter(stem.charAt(lastLetterPos + 1))) {
      lastLetterPos++;
    }
    ...
    while (done != -1) {
      ...
      if (isLetter(lastLetter) && (lastLetter >= 'a') && (lastLetter <= 'z')) {
      ...
      while (done == 0) {
        ...
        while (ruleOK == 0) {
          ...
          else if (rule.charAt(ruleCharPos) != stem.charAt(wordCharPos)) {
            ruleOK = -1;
          }
        ...
        if (ruleOK == 1) {
          ...
          while (!((rule.charAt(ruleCharPos + replacedCharCount + 1) >= '.')
              && (rule.charAt(ruleCharPos + replacedCharCount + 1) <= '>'))) {
            replacedCharCount++;
          }

          replacedCharCount =
              lastLetterPos + replacedCharCount + zeroDigit - ((int) (rule.charAt(ruleCharPos)));
        ...
        if (ruleOK == 1) {
          ...
          lastLetterPos = lastLetterPos + zeroDigit - ((int) (rule.charAt(ruleCharPos)));
          ...
          //  to the string.
          while ((ruleCharPos < rule.length()) && isLetter(rule.charAt(ruleCharPos))) {
   ...
   *     If so, "y" is not considered a vowel.
   */
  ...
  protected String stripPrefixes(String s) {
    ...
    for (int i = 0; i < prefixes.length; i++) {
      if ((sLower.startsWith(prefixes[i])) && (sLower.length() > prefixes[i].length())) {
