...
public class TextTiling {
   ...
   */
  protected double blockCosine(final Map<String, Integer> B1, final Map<String, Integer> B2) {
    //  1. Declare variables
    ...
    for (Iterator<String> e = union.keySet().iterator(); e.hasNext(); ) {
      term = e.next();
  ...
  public void boundaryIdentification() {
    ...
    for (int i = pseudo_boundaries.size(); i-- > 0; ) {
      pseudo_boundary = ((Integer) pseudo_boundaries.get(i)).intValue();
  ...
  public void similarityDetermination() {
    ...
    for (i = w; i < end; i++) {
      ...
      if (include(i - w)) {
        blockRemove(stemOf.get(text.get(i - w)), left);
      }
      ...
      if (include(i + w)) {
        blockAdd(stemOf.get(text.get(i + w)), right);
      }
    ...
    for (int j = 0; j < sim_score.length; j++) {
      sim_score[j] =
          (((Float) score.get(j)).doubleValue()
                  ...
                  + ((Float) score.get(j + 2)).doubleValue())
              / 3;
