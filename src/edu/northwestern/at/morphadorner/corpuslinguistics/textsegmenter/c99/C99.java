 ...
 *     <p>Use of this code is free for academic, education, research and other non-profit making
 *     uses only.
 */
public class C99 {
  /** Text segment region. */
    ...
    public static void bestBoundary_max(final double[][] m, Region r) {
      ...
      if (start < end) {
        ...
        for (int i = end, lower = end - start, upper = 1; (i--) > start; lower--, upper++) {
          ...
          d = (m[i][start] + m[end][i + 1]) / (double) ((lower * lower) + (upper * upper));
   ...
   *
   * @param m Similarity matrix.
   * @param n Number of regions to find. If n = 1, the algorithm will determine the number of
   *     regions.
  ...
  protected static int[] boundaries(final double[][] m, final int n) {
        ...
        if (r.start < r.end) {
          ...
          tempD =
              (sum_region - r.sum + r.left.sum + r.right.sum)
                  / (double) (sum_area - r.area + r.left.area + r.right.area);
    ...
    if (n != -1) {
      ...
      //  Smooth gradient.
      dD = Convolution.convolute(dD, new double[] {1, 2, 4, 8, 4, 2, 1});
      ...
      double threshold = dist.getMean() + 1.2D * dist.getStandardDeviation();
  ...
  protected static ContextVector[] normalize(
      final String[][] document, StopWords stopWords, Stemmer stemmer) {
    ...
    for (int i = document.length; (i--) > 0; ) {
      ...
      for (int j = document[i].length; (j--) > 0; ) {
        ...
        if (CharUtils.isAWord(token) && !stopWords.isStopWord(token)) {
   ...
   */
  protected static ContextVector[] normalize(
      final String[][] document, ContextVector tf, StopWords stopWords, Stemmer stemmer) {
    ContextVector[] v = new ContextVector[document.length];
   ...
   *     <p>Hard ranking replaces a pixel value with the proportion of neighboring values it
   *     exceeds, using a maskSize x maskSize size mask.
   */
  protected static double[][] rank(final double[][] f, final int maskSize) {
    double[][] m = new double[f.length][f.length];

    //  Compute the offset used for mask.

    final int dS = ((maskSize % 2) == 1 ? maskSize / 2 : (maskSize - 1) / 2);
   ...
   *
   * @param document Document text as list of elementary text blocks.
   * @param n Number of topic segments desired. Set n = -1 to have algorithm select number of topic
   *     segments by monitoring the rate of increase in segment density.
   * @param s Size of ranking mask. Must be odd number >= 3.
   * @param stopWords Stop words.
   * @param stemmer Stemmer.
   * @return Coherent topic segments.
   */
  public static String[][][] segment(
      final String[][] document, final int n, final int s, StopWords stopWords, Stemmer stemmer) {
  ...
  public static String[][][] segmentW(
    ...
    ContextVector[] vectors = normalize(document, tf, stopWords, stemmer);
   ...
   * @return similarity matrix.
   */
  protected static double[][] similarity(final ContextVector[] v, final EntropyVector entropy) {
    double[][] similarity = new double[v.length][v.length];
   ...
   */
  protected static String[][][] split(final String[][] text, final int[] boundaries) {
  ...
  protected static double[][] sum(final double[][] rankMatrix) {
    ...
    for (int i = 0, ie = rankMatrix.length - 1, ip; i < ie; i++) {
      ...
      sum[ip][i] = rankMatrix[ip][i] * 2.0D + sum[i][i] + sum[ip][ip];
    ...
    for (int j = 2, ij, ip; j < rankMatrix.length; j++) {
      for (int i = 0, ie = rankMatrix.length - j; i < ie; i++) {
        ...
        sum[ij][i] = rankMatrix[ij][i] * 2.0D + sum[ij - 1][i] + sum[ij][ip] - sum[ij - 1][ip];
