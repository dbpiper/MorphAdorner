 ...
 *
 * <p>Subclasses must implement {@link #replace} to perform the actual replacement.
 *
 ...
 */
public abstract class AbstractRegexReplacementRule implements WordRule {
   ...
   * @throws IllegalArgumentException when the word does not match the pattern.
   */
  public String apply(String word) {
    ...
    if (!matcher.matches()) {
      throw new IllegalArgumentException(
          "Word '" + word + "' does not match regex: " + pattern.pattern());
    }
   ...
   *     <p>For example, if "patterns" contains "a" and "b", then the disjunction is "(a|b)", that
   *     is, "a or b".
   */
  ...
  public static String disjunction(Set<String> patterns) {
    return disjunction(patterns.toArray(new String[patterns.size()]));
  }
