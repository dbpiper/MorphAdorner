...
/**
 * A {@link Pluralizer} implemented using an ordered list of {@link
 * edu.northwestern.at.morphadorner.corpuslinguistics.inflector.wordrule.WordRule}s.
 *
 * <p>You may specify a fallback {@link Pluralizer} that is invoked when none of the rules match.
 * This allows you to override some rules of another {@link Pluralizer}.
 *
 * <p>This class preserves leading and trailing whitespace, so individual rules don't need to
 * explicitly handle whitespace.
 *
 * <p>Case is also preserved -- that is, the output of all uppercase input is automatically
 * uppercased, and the output of titlecase input is automatically titlecased. This means rules can
 * act in a case-insensitive manner.
 *
 * <p>Original code written by Tom White under the Apache v2 license. Modified by Philip R. Burns
...
public class RuleBasedPluralizer implements Pluralizer {
   ...
   *     <p>A noop pluralizer is used as the fall back pluralizer when none of the specified rules
   *     applies.
   */
   ...
   *     <p>The fall back pluralizer is invoked when none of the specified rules applies.
   */
   ...
   * @return Plural form of the noun or pronoun, or <code>null</code> when no rule matches.
   */
   ...
   *     <p>If <code>trimmedWord</code> is all uppercase, then <code>pluralizedWord</code> is
   *     uppercased. If <code>trimmedWord</code> is titlecase, then <code>pluralizedWord</code> is
   *     titlecased.
   */
