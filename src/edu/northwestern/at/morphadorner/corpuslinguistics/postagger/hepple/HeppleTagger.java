 ...
 *
 * <p>Copyright (c) 2001-2005, The University of Sheffield.
 *
 * <p>This file is part of GATE (see http://gate.ac.uk/), and is free software, licenced under the
 * GNU Library General Public License, Version 2, June 1991 (in the distribution as file
 * licence.html, and also available at http://gate.ac.uk/gate/licence.html).
 *
 * <p>HeppleTagger was originally written by Mark Hepple. The GATE version contains modifications by
 * Valentin Tablan and Niraj Aswani.
 *
 * <p>This version also contains many modifications made at Northwestern University for use in the
 * WordHoard project.
 *
 * <p>Comments:
 *
 * <p>Implements a version of the decision list based tagging method described in:
 *
 * <p>M. Hepple. 2000. Independence and Commitment: Assumptions for Rapid Training and Execution of
 * Rule-based Part-of-Speech Taggers. Proceedings of the 38th Annual Meeting of the Association for
 * Computational Linguistics (ACL-2000). Hong Kong, October 2000.
 *
 * <p>Modified by Philip R. Burns at Northwestern University to remove dependencies upon the Penn
 * Treebank tag set, to allow plugable handling of unknown words, to remove all input/output for
 * tagged text and rules to calling classes, and to allow the Hepple tagger to be used as a
 * retagger.
 */
public class HeppleTagger extends AbstractPartOfSpeechTagger
   ...
   * Tagging rules.
   *
   * <p>The tagging rules are stored in a map. The map keys are parts of speech. The value for each
   * part of speech key is a lists of rules which apply to that part of speech.
   *
   * <p>Tagging rules are specified using the syntax proposed by Eric Brill in his dissertation.
   ...
   * </blockquote>
   *
   * <p>where "fromtag" is the current tag for a word, "totag" is the new tag to replace the current
   * tag if the "condition" is met, and "param1" and "param2" are optional values for the condition
   * test. Each rule must specify at least the fromtag. totag, and condition. The fromtag values are
   * the keys for the rules map.
   */
  ...
  public String[][] lexBuff = {
    staartLex, staartLex, staartLex, staartLex, staartLex, staartLex, staartLex
  };
   ...
   * @param taggedSentence A List of adorned words representing the results of tagging the current
   *     sentence so far.
   * @return true if a full sentence is now tagged, false otherwise.
   *     <p>Adds a new word to the current window of 7 words (on the last position) and tags the
   *     word currently in the middle (i.e. on position 3). This function also reads the word on the
   *     first position and adds its tag to the taggedSentence structure as this word would be lost
   *     at the next advance. If this word completes a sentence then it returns true otherwise it
   *     returns false.
   */
  ...
  protected boolean oneStep(
    ...
    //  no rule applies.
    if (debug) {
      System.out.println("===> word=" + wordBuff[3] + " currently tagged " + tagBuff[3]);
    }
    ...
    if (taggedWord != staart) {
      AdornedWord newWord = new BaseAdornedWord(taggedWord, tagBuff[0]);
  ...
  protected <T extends AdornedWord> boolean oneRetagStep(
      T adornedWord, boolean isFirstWord, List<T> taggedSentence) {
    //  Add the new word at the end of the
    //  text window.
    ...
    lexBuff[6] = getPartsOfSpeech(adornedWord.getSpelling(), false);
    ...
    if (taggedWord != staart) {
      AdornedWord aWord = new BaseAdornedWord(taggedWord, tagBuff[0]);
   ...
   *     <p>The lexicon must always return one or more parts of speech. In addition, for this
   *     tagger, the most frequently occurring tag must be the first one in the returned string
   *     array.
   */
