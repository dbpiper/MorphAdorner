...
/**
 * {@link Detector} class is to detect language from specified text. Its instance is able to be
 * constructed via the factory class {@link DetectorFactory}.
 *
 * <p>After appending a target text to the {@link Detector} instance with {@link #append(Reader)} or
 * {@link #append(String)}, the detector provides the language detection results for target text via
 * {@link #detect()} or {@link #getProbabilities()}. {@link #detect()} method returns a single
 * language name which has the highest probability. {@link #getProbabilities()} methods returns a
 * list of multiple languages and their probabilities.
 *
 * <p>The detector has some parameters for language detection. See {@link #setAlpha(double)}, {@link
 * #setMaxTextLength(int)} and {@link #setPriorMap(HashMap)}.
 *
 * <pre>
 * import java.util.ArrayList;
 ...
 *
 * <ul>
 *   <li>4x faster improvement based on Elmer Garduno's code. Thanks!
 * </ul>
 *
...
public class Detector {
  ...
  private static final Pattern MAIL_REGEX =
      Pattern.compile("[-_.0-9A-Za-z]{1,64}@[-_0-9A-Za-z]{1,255}[-_.0-9A-Za-z]{1,255}");
  ...
  public void setPriorMap(HashMap<String, Double> priorMap) throws LangDetectException {
      ...
      if (priorMap.containsKey(lang)) {
        ...
        if (p < 0)
          throw new LangDetectException(
              ErrorCode.InitParamError, "Prior probability must be non-negative.");
        this.priorMap[i] = p;
        sump += p;
      }
    }
    if (sump <= 0)
      throw new LangDetectException(
          ErrorCode.InitParamError, "More one of prior probability must be non-zero.");
    for (int i = 0; i < this.priorMap.length; ++i) this.priorMap[i] /= sump;
  }

  /**
   * Specify max size of target text to use for language detection. The default value is
   * 10000(10KB).
   *
  ...
  /**
   * Append the target text for language detection. This method read the text from specified input
   * reader. If the total size of target text exceeds the limit size specified by {@link
   * Detector#setMaxTextLength(int)}, the rest is cut down.
   *
  ...
  /**
   * Append the target text for language detection. If the total size of target text exceeds the
   * limit size specified by {@link Detector#setMaxTextLength(int)}, the rest is cut down.
   *
  ...
  /**
   * Cleaning text to detect (eliminate URL, e-mail address and Latin sentence if it is not written
   * in Latin alphabet)
   */
  ...
  /**
   * Detect language of the target text and return the language name which has the highest
   * probability.
   *
   * @return detected language name which has most probability.
   * @throws LangDetectException code = ErrorCode.CantDetectError : Can't detect because of no valid
   *     features in text
   */
   ...
   *
   * @return possible languages list (whose probabilities are over PROB_THRESHOLD, ordered by
  ...
  /**
   * Initialize the map of language probabilities. If there is the specified prior map, use it as
   * initial map.
   *
   ...
   *
   * @param word
   * @return Input word with unicode characters above 127 converted to \\uxxxx format.
   */
