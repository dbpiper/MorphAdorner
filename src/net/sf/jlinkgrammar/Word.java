...
/**
 * The basic word class TODO - Create word comparator and internationalization. That is: If the word
 * is in French and we need to know if it is a conjunction we need something like: Jaque et Pierre
 * == John and Peter Word.compare(english("and"));
 */
public class Word {
  /**
   * Looks up the word s in the dictionary. Returns null if it's not there. If there, it builds the
   * list of expressions for the word, and returns a pointer to it.
   *
  ...
  private void special_string(String s, Dictionary dict) {
    ...
    if (dict.boolean_dictionary_lookup(s)) {
      ...
      throw new RuntimeException(
          "Can't build expressions. To process this sentence your dictionary needs the word \""
              + s
              + "\"");
    }
  }
  ...
  /**
   * indicates that the first character is upper case TODO - Remove English (Indo European)
   * dependancy
   */
