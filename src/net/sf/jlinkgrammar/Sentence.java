...
/**
 * Routines for creating and destroying processing Sentences. It contains several its own copy of
 * objects such as Dictionary, ParseInfo, Linkage, LinkageInfo, ParseOptions, and Postprocessor.
 * Much of the code still smacks of C rather than Java. For instance the critical ctable is not a
 * collection object and ctable_length can be modified independently of the actual lenght of the
 * table.
 *
 * <p>There are other examples, many routines take ParseOptions as an argument and yet use this.opts
 * for their use. If the programmer is not careful odd results may occur.
 *
 * <p>One last note: Everything is reference via integer indexes into arrays rather than as objects.
 * A natural object oriented approach would pass the Word object and the routine would use Word.id
 * to find the offset in the array. This would improve type checking and overall program safety. -
 * jlr
 *
 * <p>The most important routine is sentence_parse()
 *
...
public class Sentence {
  ...
  /**
   * total number of linkages allocated. the number post-processed might be fewer because some are
   * non-canonical
   */
  public int num_linkages_alloced;

  /**
   * The number of linkages that are actually put into the array that was alloced. this is not the
   * same as num alloced because some may be non-canonical.
   */
  ...
  /**
   * This just looks up all the words in the sentence, and builds up an appropriate error message in
   * case some are not there. It has no side effect on the sentence. Returns true if all went well.
   */
    ...
    for (w = 0; w < word.size(); w++) {
      ...
      if (!dict.boolean_dictionary_lookup(s)
          && !(Character.isUpperCase(s.charAt(0))
              && Dictionary.is_s_word(s)
              && dict.pl_capitalized_word_defined)
  ...
  /**
   * The string s has just been read in from standard input. This function breaks it up into words
   * and stores these words in the sent.word[] array. Quote marks are treated just like blanks.
   *
   * @param s sentence in String form
   * @param opts passes ParseOptions - In reality these are often kept in global variables. TODO -
   *     clean up code
   * @see ParseOptions
  ...
  public boolean separate_sentence(String s, ParseOptions opts) {
    ...
    for (int i = 0; i < GlobalBean.MAX_SENTENCE; i++) post_quote[i] = 0;

    if (dict.left_wall_defined) if (!issue_sentence_word(GlobalBean.LEFT_WALL_WORD)) return false;
    ...
    for (; ; ) {
      ...
      while (j < s.length() && (Character.isWhitespace(s.charAt(j)) || s.charAt(j) == '\"')) {
        if (s.charAt(j) == '\"') quote_found = true;
        j++;
      }
      if (j >= s.length()) break;
      int k = j;
      while (k < s.length() && !(Character.isWhitespace(s.charAt(k)) || s.charAt(k) == '\"')) k++;
      if (!separate_word(s.substring(j), k - j, is_first, quote_found, opts)) return false;
      is_first = false;
      j = k;
      if (j >= s.length()) break;
    }

    if (dict.right_wall_defined) if (!issue_sentence_word(GlobalBean.RIGHT_WALL_WORD)) return false;

    return (word.size() > (dict.left_wall_defined ? 1 : 0) + (dict.right_wall_defined ? 1 : 0));
  }

  /**
   * w points to a string, wend points to the char one after the end. The "word" w contains no
   * blanks. This function splits up the word if necessary, and calls "issue_sentence_word()" on
   * each of the resulting parts. The process is described above. returns true of OK, false if too
   * many punctuation marks
   */
  private boolean separate_word(
      String w, int wend, boolean is_first_word, boolean quote_found, ParseOptions opts) {
    ...
    if (dict.affix_table != null) {
      ...
      for (dn = start_dn; dn != null; dn = dn.right) {
        if (dn.word_has_connector(rpunc_con, 0)) r_strippable++;
        if (dn.word_has_connector(lpunc_con, 0)) l_strippable++;
        if (dn.word_has_connector(suf_con, 0)) s_strippable++;
        if (dn.word_has_connector(pre_con, 0)) p_strippable++;
      }
    ...
    for (n_r_stripped = 0; n_r_stripped < GlobalBean.MAX_STRIP; n_r_stripped++) {
      ...
      if (word.length() == 0) break; /* it will work without this */

      if (dict.boolean_dictionary_lookup(word.toString())
          || Dictionary.is_initials_word(word.toString())) break;
      ...
      for (i = 0; i < r_strippable; i++) {
        ...
        if (wend < len) continue; /* the remaining w is too short for a possible match */
    ...
    if (dict.boolean_dictionary_lookup(word.toString())
        || Dictionary.is_initials_word(word.toString())) word_is_in_dict = true;
    if (is_first_word && Character.isUpperCase(word.charAt(0))) {
      word.setCharAt(0, Character.toLowerCase(word.charAt(0)));
      if (dict.boolean_dictionary_lookup(word.toString())) {
    ...
    if (!word_is_in_dict) {
      ...
      for (i = 0; i <= s_strippable; i++) {
        ...
        if (i < s_strippable) {
          len = suffix[i].length();
          if (wend < len) continue; /* the remaining w is too short for a possible match */
          if (word.toString().endsWith(suffix[i])) s_ok = true;
        } else len = 0;
                  ...
                  if (opts.verbosity > 1 && i < s_strippable)
                    opts.out.println(
                        "Splitting word into three: "
                            ...
                            + "-"
                            + suffix[i]);
                  if (!issue_sentence_word(prefix[j])) return false;
    ...
    if (quote_found) post_quote[this.word.size()] = 1;

    if (!issue_sentence_word(word.toString())) return false;

    if (s_stripped != -1) {
      if (!issue_sentence_word(suffix[s_stripped])) return false;
    }

    for (i = n_r_stripped - 1; i >= 0; i--) {
      if (!issue_sentence_word(strip_right[r_stripped[i]])) return false;
    }
  ...
  /**
   * the string s is the next word of the sentence do not issue the empty string.
   *
  ...
  /**
   * Corrects case of first word, fills in other proper nouns, and builds the expression lists for
   * the resulting words.
   *
   * <p>Algorithm:
   *
   ...
   *   <li>else leave the disjuncts alone </us>
   *       <p>Here's a summary of how subscripts are handled:
   *       <p>Reading the dictionary:
   *       <p>If the last "." in a string is followed by a non-digit character, then the "." and
   *       everything after it is considered to be the subscript of the word.
   *       <p>The dictionary reader does not allow you to have two words that match according to the
   *       criterion below. (so you can't have "dog.n" and "dog")
   *       <p>Quote marks are used to allow you to define words in the dictionary which would
   *       otherwise be considered part of the dictionary, as in
   *       <p>";": {&#064Xca-} & Xx- & (W+ or Qd+) & {Xx+}; "%" : (ND- & {DD-} & \<noun-sub-x\> &
   *       (\<noun-main-x\> or B*x+)) or (ND- & (OD- or AN+));
   *       <p>Rules for chopping words from the input sentence:
   *       <p>First the prefix chars are stripped off of the word. These characters are "(" and "$"
   *       (and now "``")
   *       <p>Now, repeat the following as long as necessary:
   *       <p>Look up the word in the dictionary. If it's there, the process terminates.
   *       <p>If it's not there and it ends in one of the right strippable strings (see
   *       "right_strip") then remove the strippable string and make it into a separate word.
   *       <p>If there is no strippable string, then the process terminates.
   *       <p>Rule for defining subscripts in input words:
   *       <p>The subscript rule is followed just as when reading the dictionary.
   *       <p>When does a word in the sentence match a word in the dictionary?
   *       <p>Matching is done as follows: Two words with subscripts must match exactly. If neither
   *       has a subscript they must match exactly. If one does and one doesn't then they must match
   *       when the subscript is removed. Notice that this is symmetric.
   *       <p>So, under this system, the dictonary could have the words "Ill" and also the word
   *       "Ill." It could also have the word "i.e.", which could be used in a sentence.
   *
  ...
  /**
   * How is the is_conjunction table initialized? TODO - Remove English dependancy Also what about
   * "yet", "however", "then", "else", "whence", "thus", ... Word word.get(w) has a list of
   * equivilent expressions that should be followed what is needed here is:
   *
   * <p>if (isConjunct(word.get(w))||isDisjunct(word.get(w))||isAdjunct(word.get(w))) then
   * is_conjunction[w] = true;
   *
   * <p>where isConjunct(Word word) walks down the list of equivilent expressions
   */
  public void set_is_conjunction() {
    ...
    for (int w = 0; w < word.size(); w++) {
      s = word.get(w).string;
      word.get(w).is_conjunction =
          ((s.equals("and")) || (s.equals("or")) || (s.equals("but")) || (s.equals("nor")));
    }
  }
  ...
  /**
   * assumes that the sentence expression lists have been generated this does all the necessary
   * pruning and building of and structures.
   *
  ...
  public void prepare_to_parse(ParseOptions opts) {
    ...
    /**
     * why do we do these here instead of in first_prepare_to_parse() only? The reason is that the
     * deletable region depends on if null links are in use. with null_links everything is deletable
     */
    if (!has_conjunction) {
      ...
      if (opts.verbosity > 2) print_AND_statistics(opts);
  ...
  /**
   * We've already built the sentence disjuncts, and we've pruned them and power_pruned(GENTLE) them
   * also. The sentence contains a conjunction. deletable[][] has been initialized to indicate the
   * ranges which may be deleted in the final linkage.
   *
   * <p>This routine deletes irrelevant disjuncts. It finds them by first marking them all as
   * irrelevant, and then marking the ones that might be useable. Finally, the unmarked ones are
   * removed.
   *
   ...
   * CONJUNCTION PRUNING.
   *
   * <p>The basic idea is this. Before creating the fat disjuncts, we run a modified version of the
   * exhaustive search procedure. Its purpose is to mark the disjuncts that can be used in any
   * linkage. It's just like the normal exhaustive search, except that if a subrange of words are
   * deletable, then we treat them as though they were not even there. So, if we call the function
   * in the situation where the set of words between the left and right one are deletable, and the
   * left and right connector pointers are null, then that range is considered to have a solution.
   *
   * <p>There are actually two procedures to implement this. One is mark_region() and the other is
   * region_valid(). The latter just checks to see if the given region can be completed (within it).
   * The former actually marks those disjuncts that can be used in any valid linkage of the given
   * region.
   *
   * <p>As in the standard search procedure, we make use of the fast-match data structure (which
   * requires power pruning to have been done), and we also use a hash table. The table is used
   * differently in this case. The meaning of values stored in the table are as follows:
   *
   * <p>-1 Nothing known (Actually, this is not stored. It's returned by table_lookup when nothing
   * is known.) 0 This region can't be completed (marking is therefore irrelevant) 1 This region can
   * be completed, but it's not yet marked 2 This region can be completed, and it's been marked.
   *
   ...
   * @return Returns 0 if this range cannot be successfully filled in with links. Returns 1 if it
   *     can, and it's not been marked, and Returns 2 if it can and it has been marked.
   */
  ...
  /**
   * Forms and returns a list of disjuncts that might match lc or rc or both. lw and rw are the
   * words from which lc and rc came respectively. The list is formed by the link pointers of
   * MatchNodes. The list contains no duplicates. A quadratic algorithm is used to eliminate
   * duplicates. In practice the match_cost is less than the parse_cost (and the loop is tiny), so
   * there's no reason to bother to fix this.
   *
  ...
  void install_fat_connectors() {
    ...
    for (int i = 0; i < word.size(); i++) {
      if (word.get(i).is_conjunction) {
        word.get(i).d =
            Disjunct.catenate_disjuncts(word.get(i).d, build_AND_disjunct_list(word.get(i).string));
      } else {
        word.get(i).d =
            Disjunct.catenate_disjuncts(word.get(i).d, explode_disjunct_list(word.get(i).d));
  ...
  public void connector_for_disjunct(Disjunct d, Connector c) {
    ...
    */
    ...
    for (lp = and_data.hash_table[h]; lp != null; lp = lp.next) {
      ...
      if (d.disjunct_types_equal(d1)) break;
    }
    if (lp == null) {
    ...
    */
    while (d1 != null) {
      if (d1.disjuncts_equal_AND(d)) break;
      d1 = d1.next;
    }
  ...
  /**
   * Builds and returns a disjunct list for "and", "or" and "nor" for each disjunct in the
   * label_table, we build three disjuncts this means that "Danny and Tycho and Billy" will be
   * parsable in two ways. I don't know an easy way to avoid this the string is either "and", or
   * "or", or "nor" at the moment
   *
   * <p>must accommodate "he and I are good", "Davy and I are good" "Danny and Davy are good", and
   * reject all of these with "is" instead of "are".
   *
   * <p>The SI connectors must also be modified to accommodate "are John and Dave here", but kill
   * "is John and Dave here"
   *
   * <p>Then we consider "a cat or a dog is here" vs "a cat or a dog are here" The first seems
   * right, the second seems wrong. I'll stick with this.
   *
   * <p>That is, "or" has the property that if both parts are the same in number, we use that but if
   * they differ, we use plural.
   *
   * <p>The connectors on "I" must be handled specially. We accept "I or the dogs are here" but
   * reject "I or the dogs is here"
   *
   * <p>TODO - the code here still does now work "right", rejecting "is John or I invited" and
   * accepting "I or my friend know what happened"
   *
   * <p>The more generous code for "nor" has been used instead
   *
   * <p>It appears that the "nor" of two things can be either singular or plural.
   *
   * <p>"neither she nor John likes dogs"
   *
   * <p>"neither she nor John like dogs"
   *
   * @param s
    ...
    */
    ...
    while (d != null) {
      ...
      for (c = d.left; c != null; c = c.next) {
        s = c.string;
        j = 0;
        while (j < s.length() && Character.isUpperCase(s.charAt(j))) j++;
        j = s.length() - j;
  ...
  public void put_disjunct_into_table(Disjunct d) {
    ...
    */
    ...
    if (lp != null) {
      ...
      while (d1 != null) {
        if (d1.disjuncts_equal_AND(d)) return;
        d1 = d1.next;
      }
      ...
      */
      ...
      for (; d2 != null; d2 = di) {
        di = d2.next;
        for (d1 = and_data.label_table[k]; d1 != null; d1 = d1.next) {
          if (d1.disjuncts_equal_AND(d2)) break;
        }
        if (d1 == null) {
      ...
      d_copy.cost = 0;
      d_copy.next = null;
      if (and_data.LT_size == and_data.LT_bound) grow_LT();
      lp = new LabelNode();
  ...
  //
  /**
   * returns true if the disjunct is appropriate to be made into fat links. Check here that the
   * connectors are from some small set. This will disallow, for example "the and their dog ran".
   *
   * <p>TODO: move to dict
   *
   * <p>
  ...
  public boolean is_appropriate(Disjunct d) {
    ...
    for (c = d.right; c != null; c = c.next) {
      if (!c.match_in_connector_set(this, dict.andable_connector_set, '+')) return false;
    }
    for (c = d.left; c != null; c = c.next) {
      if (!c.match_in_connector_set(this, dict.andable_connector_set, '-')) return false;
    }
  ...
  public void print_AND_statistics(ParseOptions opts) {
    ...
    if (and_data.LT_size != 0) {
      opts.out.println(
          "average list word.size(): " + ((float) GlobalBean.STAT_N_disjuncts / and_data.LT_size));
    }
    ...
    if (null_links) {
      ...
      for (i = 0; i < word.size(); i++) {
        for (j = i + 1; j < word.size(); j++) {
          if (word.get(i).is_conjunction || word.get(j).is_conjunction) effective_dist[i][j] = 1;
        }
      }
    }
  }
  ...
  /**
   * Initialize the array deletable[i][j] to indicate if the words i+1...j-1 could be non existant
   * in one of the multiple linkages. This array is used in conjunction_prune and power_prune.
   * Regions of word.size() 0 are always deletable. A region of word.size() two with a conjunction
   * at one end is always deletable. Another observation is that for the comma to form the right end
   * of a deletable region, it must be the case that there is a conjunction to the right of the
   * comma. Also, when considering deletable regions with a comma on their left sides, there must be
   * a conjunction inside the region to be deleted. Finally, the words "either", "neither", "both",
   * "not" and "not only" are all deletable.
   *
   * <p>TODO - This is awfully ethnocentric. What about other languages, or words like thus, thence,
   * whence etc. This should be a loadable array!
   *
   * @param has_conjunction
   */
        ...
        if (j == i + 1) {
          ...
          for (k = i + 1; k < j; k++) {
            if ("either".equals(word.get(k).string)
                ...
                || "not".equals(word.get(k).string)) continue;
            if (("only".equals(word.get(k).string))
                && (k > i + 1)
                && ("not".equals(word.get(k - 1).string))) continue;
            break;
          }
          deletable[i + 1][j] = (k == j);
        } else {
  ...
  public boolean conj_in_range(int lw, int rw) {
    for (; lw <= rw; lw++) {
      if (word.get(lw).is_conjunction) return true;
    }
    return false;
  }
  ...
  private void set_connector_length_limits(ParseOptions opts) {
    ...
    if (len > GlobalBean.UNLIMITED_LEN) len = GlobalBean.UNLIMITED_LEN;
    for (i = 0; i < word.size(); i++) {
  ...
  private void set_connector_list_length_limit(
      Connector c, ConnectorSet conset, int short_len, ParseOptions opts) {
    for (; c != null; c = c.next) {
  ...
  /**
   * Step three in parsing a sentence. First you must create a dictionary, then a sentence, then
   * call this method to generate a parse tree. This is really the heart of the system. There are
   * several things done when a sentence is parsed.
   *
   * <ul>
   ...
   * </ul>
   *
   * The "parse set" is attached to the sentence, and this is one of the key reasons that the API is
   * flexible and modular. All of the necessary information for building linkages is stored in the
   * parse set. This means that other sentences can be parsed, possibly using different
   * dictionaries, without disturbing the information obtained from a call to sentence_parse. If
   * another call to sentence_parse is made on the same sentence, the parsing information for the
   * previous call is deleted.
   *
   * <p>O.K. that may be true of the C code version but in this code a lot of information from
   * ParseOptions is held in GlobalBean.
   *
   * <p>TODO - Make the dictionary and ParseInfo private to the sentence. Then add getter and setter
   * methods.
   *
   * @param opts
  ...
  /**
   * Returns the number of null links the sentence can be parsed with the specified cost Assumes
   * that the hash table this.ctable has already been initialized, and is freed later.
   *
   * @param cost
   * @param opts
    ...
    for (w = start_word; w <= end_word; w++) {
      ...
      for (; m != null; m = m.next) {
        ...
        for (lcost = 0; lcost <= cost; lcost++) {
          ...
          pseudototal =
              leftcount * rightcount; /* total number where links are used on both sides */
          ...
          if (pseudototal != 0) {
            ...
            if (Lmatch) {
              ...
              if (le.multi) leftcount += count(lw, w, le, d.left.next, lcost, opts);
              if (d.left.multi) leftcount += count(lw, w, le.next, d.left, lcost, opts);
              if (le.multi && d.left.multi) leftcount += count(lw, w, le, d.left, lcost, opts);
            }

            if (Rmatch) {
              rightcount = count(w, rw, d.right.next, re.next, rcost, opts);
              if (d.right.multi) rightcount += count(w, rw, d.right, re.next, rcost, opts);
              if (re.multi) rightcount += count(w, rw, d.right.next, re, rcost, opts);
              if (d.right.multi && re.multi) rightcount += count(w, rw, d.right, re, rcost, opts);
            }
  ...
  /**
   * A piecewise exponential function determines the size of the hash table. Probably should make
   * use of the actual number of disjuncts, rather than just the number of words
   */
  ...
  /**
   * The size of this.ctable TODO - make this Java, not C, and use the collection object stuff! sp
   * that ctable size can not be modified independently of ctable.
   */
  ...
  public static int hash(int lw, int rw, Connector le, Connector re, int cost) {
    ...
    i =
        i
            + (i << 1)
            + MyRandom.randtable[
                (((le == null ? 0 : le.hashCode()) + i) % (Sentence.ctable_size + 1))
                    & (GlobalBean.RTSIZE - 1)];
    i =
        i
            + (i << 1)
            + MyRandom.randtable[
                (((re == null ? 0 : re.hashCode()) + i) % (Sentence.ctable_size + 1))
                    & (GlobalBean.RTSIZE - 1)];
   ...
   * @see #init_table()
   */
  public static TableConnector table_store(
      int lw, int rw, Connector le, Connector re, int cost, int count) {
  ...
  public static int match_l_table_size[] =
      new int[GlobalBean.MAX_SENTENCE]; /* the sizes of the hash tables */
  ...
  /**
   * @param d
   * @return the number of disjuncts in the list that have non-null left connector lists
   */
   ...
   *
   * @param d
   * @return the number of disjuncts in the list that have non-null right connector lists
   */
   ...
   *
   * <p>dir = 1, we're putting this into a right table.
   *
   * <p>dir = -1, we're putting this into a left table.
   *
   * @param size
   * @param t
   * @param d
   ...
   * @param dir
   */
  public static void put_into_match_table(
      int size, MatchNode t[], Disjunct d, Connector c, int dir) {
  ...
  /**
   * This hash function only looks at the leading upper case letters of the connector string, and
   * the label fields. This ensures that if two strings match (formally), then they must hash to the
   * same place. The answer must be masked to the appropriate table size.
   *
   * @param c
  ...
  /**
   * Adds the match node m to the sorted list of match nodes l. The parameter dir determines the
   * order of the sorting to be used. Makes the list sorted from smallest to largest.
   *
  ...
  /**
   * Adds the match node m to the sorted list of match nodes l. The parameter dir determines the
   * order of the sorting to be used. Makes the list sorted from largest to smallest
   *
   * @param m the node to add
   * @param l the node to which we are to add m on the right
  ...
  /**
   * This is the top level call that computes the whole parse_set. It points whole_set at the
   * result. It creates the necessary hash table (x_table) which will be freed at the same time the
   * whole_set is freed.
   *
   * <p>It also assumes that count() has been run, and that hash table is filled with the values
   * thus computed. Therefore this function must be structured just like parse() (the main function
   * for count()).
   *
   * <p>If the number of linkages gets huge, then the counts can overflow. We check if this has
   * happened when verifying the parse set. This routine returns true iff overflowed occurred.
   *
   * <p>This method modifies this.loca-sent, this.parse_info
   *
   * @param cost
   * @param opts
  ...
  /**
   * We've already built the sentence expressions. This turns them into disjuncts. The method
   * modifies this.word by adding the disjuncts to word[index].d
   *
   * @param opts - refers to this.cost_cutoff that is set from ParseInfo pi at object creation. TODO
   *     - Fix where ParseInfo is kept.
   * @param cost_cutoff
   * @see #word
   * @see Word#d
   */
  ...
  /**
   * @param opts unused - refers to this.cost_cutoff that is set from ParseInfo pi at object
   *     creation. TODO - Fix where ParseInfo is kept.
  ...
  /**
   * We've already built the sentence expressions. This turns them into disjuncts. Assumes
  ...
  public boolean sentence_contains_conjunction() {
    ...
    */
    ...
    int w;
    for (w = 0; w < word.size(); w++) {
      if (word.get(w).is_conjunction) return true;
    }
    return false;
  }
  ...
  /**
   * This is another top level call. It is called by default when you create a new Linkage object.
   * If you want another postprocessor this is the method to call.
   *
   * @param opts
  ...
  public void post_process_linkages(ParseOptions opts) {
    ...
    if (word.size() >= opts.twopass_length) {
      for (in = 0; in < N_linkages_alloced; in++) {
        ...
        if (set_has_fat_down()) {
          if (only_canonical_allowed && !is_canonical_linkage()) continue;
    ...
    for (in = 0; in < N_linkages_alloced; in++) {
      ...
      if (set_has_fat_down()) {
        ...
        link_info[N_linkages_post_processed] =
            analyze_thin_linkage(opts, GlobalBean.PP_SECOND_PASS);
    ...
    if (opts.verbosity > 1) {
      opts.out.println(
          ""
              ...
              + " of "
              + N_linkages_post_processed
              + " linkages with no P.P. violations");
    }
  ...
  /**
   * This uses link_array. It enumerates and post-processes all the linkages represented by this
   * one. We know this contains at least one fat link.
   *
   * @param opts
   * @param analyze_pass
  ...
  /**
   * During a first pass (prior to actual post-processing of the linkages of a sentence), call this
   * once for every generated linkage. Here we simply maintain a set of "seen" link names for rule
   * pruning later on
   *
   * @param pp
   * @param opts
   * @param sublinkage
   */
  public void post_process_scan_linkage(
      Postprocessor pp, ParseOptions opts, Sublinkage sublinkage) {
    ...
    int i;
    if (pp == null) return;
    if (word.size() < opts.twopass_length) return;
  ...
  /**
   * call this (a) after having called post_process_scan_linkage() on all generated linkages, but
   * (b) before calling post_process() on any particular linkage. Here we mark all rules which we
   * know (from having accumulated a set of link names appearing in *any* linkage) won't ever be
   * needed.
   *
   * @param opts
   * @param pp
  ...
  public void prune_irrelevant_rules(ParseOptions opts, Postprocessor pp) {
    ...
    if (PPLinkset.PPLinkset_population(pp.set_of_links_of_sentence) == 0) return;
    ...
    if (opts.verbosity > 1) {
      ...
      opts.out.println(
          "Using " + rcoIDX + " 'contains one' rules and " + rcnIDX + " 'contains none' rules");
    }
  }
   ...
   * </ul>
   *
   * <p>NB: sublinkage.link[i].l=-1 means that this connector is to be ignored
   *
   * @param pp
   * @param opts
   * @param sublinkage
   * @param cleanup
   * @return a PPNode for the linkage in the given postprocessor or null if none or the
   *     postprocessor is not valid
   */
  public PPNode post_process(
      Postprocessor pp, ParseOptions opts, Sublinkage sublinkage, boolean cleanup) {
    /* Takes a sublinkage and returns:
    ...
    if (cleanup) Postprocessor.post_process_free_data(pp.pp_data);
    return pp.pp_node;
  }

  /**
   * This takes as input link_array[], sublinkage.link[].l and sublinkage.link[].r (and also
   * has_fat_down[word], which has been computed in a prior call to is_canonical()), and from these
   * computes sublinkage.link[].lc and .rc. We assume these have been initialized with the values
   * from link_array. We also assume that there are fat links.
   *
   * @param sublinkage
   */
    ...
    */
    ...
    for (end = -1; end <= 1; end += 2) {
      for (link = 0; link < pi.N_links; link++) {
        ...
        if (this_end_con.label != GlobalBean.NORMAL_LABEL) continue;
  ...
  /**
   * This fills in the sublinkage.link[].name field. We assume that link_array[].name have already
   * been filled in. As above, in the standard case, the name is just the GCD of the two end points.
   * If pluralization has occurred, then we want to use the name already in link_array[].name. We
   * detect this in two ways. If the endpoints don't match, then we know pluralization has occured.
   * If they do, but the name in link_array[].name is *less* restrictive, then pluralization must
   * have occured.
   *
   * @param sublinkage
   */
    ...
    for (i = 0; i < pi.N_links; i++) {
      ...
      if (!Connector.x_match(this, sublinkage.link[i].lc, sublinkage.link[i].rc))
        ...
        if (Sentence.strictly_smaller_name(s, pi.link_array[i].name))
          sublinkage.link[i].replace_link_name(pi.link_array[i].name);
        else sublinkage.link[i].replace_link_name(s);
      }
    }
  }
   ...
   * and-list elements. It also computes other information used to construct the "andlist" structure
   * of linkage_info.
   *
  ...
  public AndList build_andlist() {
    ...
    for (w = 0; w < pi.N_words; w++) {
      if (word.get(w).is_conjunction) {
        ...
        if (dict.left_wall_defined) visited[0] = true;
        and_dfs_commas(w);
  ...
  /**
   * This uses link_array. It post-processes this linkage, and prints the appropriate thing. There
   * are no fat links in it.
   *
   * <p>The code can be used to generate the "islands" array. For this to work, however, you have to
   * call "build_digraph" first (as in analyze_fat_linkage). and then "free_digraph". For some
   * reason this causes a space leak.
   *
   * @param opts
   * @param analyze_pass
  ...
  /**
   * uses link_array[], chosen_disjuncts[], has_fat_down[]. It assumes that there is a fat link in
   * the current linkage. See AndData() for more information about how it works
   *
      ...
      for (dis = and_data.label_table[d_label]; dis != null; dis = dis.next) {
        ...
        for (in = image_array[w]; in != null; in = in.next) {
          ...
          if (place == 0) {
            ...
            if (!Connector.x_match(this, c, in.c)) break; /* Ditto above comment  --DS 07/97*/
          } else {
            for (c = dis.left; place < -1; place++) {
              c = c.next;
            }
            if (!Connector.x_match(this, c, in.c)) break; /* Ditto Ditto */
          }
        }
   ...
   * @param s
   * @param t
   * @return true if string s represents a strictly smaller match set than does t
   */
    ...
    */
    ...
    for (d = and_data.label_table[label]; d != null; d = d.next) {
      ...
      for (cx = d.right, cy = dis.right; cx != null; cx = cx.next, cy = cy.next) {
        /*      if ((cx.string != cy.string) || */
        if (!cx.string.equals(cy.string) || cx.multi != cy.multi) break;
      }
      if (cx == null) break;
    }
    if (d == null) {
  ...
  */
  ...
  public void build_image_array() {
    ...
    for (end = -1; end <= 1; end += 2) {
      for (link = 0; link < pi.N_links; link++) {
        ...
        if (this_end_con.priority == GlobalBean.DOWN_priority) continue;
        if ((this_end_con.label != GlobalBean.NORMAL_LABEL) && (this_end_con.label < 0)) continue;
  ...
  /**
   * This removes the expressions that are empty from the list corresponding to word w of the
   * sentence.
   *
   * @param w
   */
  ...
  public void expression_prune(ParseOptions opts) {
    ...
    for (; ; ) {
      ...
      free_S();
      if (N_deleted == 0) break; /* right-to-left pass */
      N_deleted = 0;
  ...
  /**
   * returns true if c can match anything in the set S because of the horrible kludge, prune match
   * is assymetric, and direction is '-' if this is an l.r pass, and '+' if an r.l pass.
   *
   * @param c
   * @param dir
  ...
  public boolean matches_S(Connector c, int dir) {
    ...
    if (dir == '-') {
      for (e = table[h]; e != null; e = e.next) {
        if (Connector.x_prune_match(this, e, c)) return true;
      }
    } else {
      for (e = table[h]; e != null; e = e.next) {
        if (Connector.x_prune_match(this, c, e)) return true;
      }
    }
    return false;
  }
    ...
    */
    ...
    for (i = 0; i < pi.N_links; i++) {
      pi.link_array[i].name =
          intersect_strings(pi.link_array[i].lc.string, pi.link_array[i].rc.string);
    }
  }
  ...
  public static String intersect_strings(String s, String t) {
    ...
    */
    ...
    StringBuffer u;
    String u0;
    if (s.equals(t)) return s; /* would work without this */
    i = s.length();
    j = t.length();
    if (j > i) {
      u0 = s;
    ...
    while (i < t.length()) {
      if (s.charAt(i) == t.charAt(i) || t.charAt(i) == '*') {
        ...
        d++;
        if (s.charAt(i) == '*') u.append(t.charAt(i));
        else u.append('^');
      }
      i++;
    }
    ...
    */
    ...
    for (w = 0; w < word.size() - 1; w++) {
      if ((word.get(w).string.equals(",")) && word.get(w + 1).is_conjunction) {
        word.get(w).d =
            Disjunct.catenate_disjuncts(
                special_disjunct(GlobalBean.COMMA_LABEL, '+', "", ","), word.get(w).d);
  ...
  */
  private void construct_either() {
    ...
    for (w = 0; w < word.size(); w++) {
      if (!word.get(w).string.equals("either")) continue;
      word.get(w).d =
          Disjunct.catenate_disjuncts(
              special_disjunct(GlobalBean.EITHER_LABEL, '+', "", "either"), word.get(w).d);
    }

    for (w = 0; w < word.size(); w++) {
      if (!word.get(w).string.equals("or")) continue;
  ...
  private void construct_neither() {
    ...
    for (w = 0; w < word.size(); w++) {
      if (!word.get(w).string.equals("neither")) continue;
      word.get(w).d =
          Disjunct.catenate_disjuncts(
              special_disjunct(GlobalBean.NEITHER_LABEL, '+', "", "neither"), word.get(w).d);
    }

    for (w = 0; w < word.size(); w++) {
      if (!word.get(w).string.equals("nor")) continue;
  ...
  private void construct_notonlybut() {
    ...
    for (w = 0; w < word.size(); w++) {
      if (!word.get(w).string.equals("not")) continue;
      word.get(w).d =
          Disjunct.catenate_disjuncts(
              special_disjunct(GlobalBean.NOT_LABEL, '+', "", "not"), word.get(w).d);
      if (w < word.size() - 1 && word.get(w + 1).string.equals("only")) {
        word.get(w + 1).d =
            Disjunct.catenate_disjuncts(
                special_disjunct(GlobalBean.NOTONLY_LABEL, '-', "", "only"), word.get(w + 1).d);
    ...
    */
    for (w = 0; w < word.size(); w++) {
      if (!word.get(w).string.equals("but")) continue;
  ...
  private void construct_both() {
    ...
    for (w = 0; w < word.size(); w++) {
      if (!word.get(w).string.equals("both")) continue;
      word.get(w).d =
          Disjunct.catenate_disjuncts(
              special_disjunct(GlobalBean.BOTH_LABEL, '+', "", "both"), word.get(w).d);
    }

    for (w = 0; w < word.size(); w++) {
      if (!word.get(w).string.equals("and")) continue;
  ...
  public boolean sentence_contains(String s) {
    ...
    int w;
    for (w = 0; w < word.size(); w++) {
      if (word.get(w).string.equals(s)) return true;
    }
    return false;
  }

  /**
   * This file contains the functions for massaging disjuncts of the sentence in special ways having
   * to do with conjunctions. The only function called from the outside world is
   * install_special_conjunctive_connectors()
   *
   * <p>It would be nice if this code was written more transparently. In other words, there should
   * be some fairly general functions that manipulate disjuncts, and take words like "neither" etc
   * as input parameters, so as to encapsulate the changes being made for special words. This would
   * not be too hard to do, but it's not a high priority. -DS 3/98
   *
   * <p>There's a problem with installing "...but...", "not only...but...", and "not...but...",
   * which is that the current comma mechanism will allow a list separated by commas. "Not only
   * John, Mary but Jim came" The best way to prevent this is to make it impossible for the comma to
   * attach to the "but", via some sort of additional subscript on commas.
   *
   * <p>I can't think of a good way to prevent this.
   *
   * <p>
   *
   * <p>The following functions all do slightly different variants of the following thing:
   *
   * <p>Catenate to the disjunct list pointed to by d, a new disjunct list. The new list is formed
   * by copying the old list, and adding the new connector somewhere in the old disjunct, for
   * disjuncts that satisfy certain conditions
   */
  ...
  /**
   * In this case the connector is to connect to the "either", "neither", "not", or some auxilliary
   * d to the current which is a conjunction. Only gets added next to a fat link, but before it (not
   * after it) In the case of "nor", we don't create new disjuncts, we merely modify existing ones.
   * This forces the fat link uses of "nor" to use a neither. (Not the case with "or".) If
   * necessary=false, then duplication is done, otherwise it isn't
   */
  ...
  /**
   * This adds one connector onto the beginning of the left (or right) connector list of d. The
   * label and string of the connector are specified
   */
  ...
  /**
   * Builds a new disjunct with one connector pointing in direction dir (which is '+' or '-'). The
   * label and string of the connector are specified, as well as the string of the disjunct. The
   * next pointer of the new disjunct set to null, so it can be regarded as a list.
   */
  ...
  public int pp_prune(ParseOptions opts) {
    ...
    if (dict.postprocessor == null) return 0;
          ...
          for (dir = 0; dir < 2; dir++) {
            for (c = (dir == 1 ? d.left : d.right); c != null; c = c.next) {
              for (i = 0; i < knowledge.n_contains_one_rules; i++) {
                ...
                if (selector.indexOf('*') >= 0) continue; /* If it has a * forget it */

                if (!Postprocessor.post_process_match(selector, c.string)) continue;
  ...
  public void pp_and_power_prune(int mode, ParseOptions opts) {
    ...
    */
    ...
    for (; ; ) {
      if (pp_prune(opts) == 0) break;
      if (power_prune(mode, opts) == 0) break;
    }
  }
   ...
   *
   * <p>The algorithms in this file prune disjuncts from the disjunct list of the sentence that can
   * be elimininated by a simple checks. The first check works as follows:
   *
   * <p>A series of passes are made through the sentence, alternating left-to-right and
   * right-to-left. Consider the left-to-right pass (the other is symmetric). A set S of connectors
   * is maintained (initialized to be empty). Now the disjuncts of the current word are processed.
   * If a given disjunct's left pointing connectors have the property that at least one of them has
   * no connector in S to which it can be matched, then that disjunct is deleted. Now the set S is
   * augmented by the right connectors of the remaining disjuncts of that word. This completes one
   * word. The process continues through the words from left to right. Alternate passes are made
   * until no disjunct is deleted.
   *
   * <p>It worries me a little that if there are some really huge disjuncts lists, then this process
   * will probably do nothing. (This fear turns out to be unfounded.)
   *
   * <p>Notes: Power pruning will not work if applied before generating the "and" disjuncts. This is
   * because certain of it's tricks don't work. Think about this, and finish this note later....
   * Also, currently I use the standard connector match procedure instead of the pruning one, since
   * I know power pruning will not be used before and generation. Replace this to allow power
   * pruning to work before generating and disjuncts.
   *
   * <p>Currently it seems that normal pruning, power pruning, and generation, pruning, and power
   * pruning (after "and" generation) and parsing take about the same amount of time. This is why
   * doing power pruning before "and" generation might be a very good idea.
   *
   * <p>New idea: Suppose all the disjuncts of a word have a connector of type c pointing to the
   * right. And further, suppose that there is exactly one word to its right containing that type of
   * connector pointing to the left. Then all the other disjuncts on the latter word can be deleted.
   * (This situation is created by the processing of "either...or", and by the extra disjuncts added
   * to a "," neighboring a conjunction.)
   *
   * <p>see AndData()
   */
   ...
   *
   * <p>The kinds of constraints it checks for are the following:
   *
   * <p>1) successive connectors on the same disjunct have to go to nearer and nearer words.
   *
   * <p>2) two deep connectors cannot attach to eachother (A connectors is deep if it is not the
   * first in its list, it is shallow if it is the first in its list, it is deepest if it is the
   * last on its list.)
   *
   * <p>3) on two adjacent words, a pair of connectors can be used only if they're the deepest ones
   * on their disjuncts
   *
   * <p>4) on two non-adjacent words, a pair of connectors can be used only if not [both of them are
   * the deepest].
   *
   * <p>The data structure consists of a pair of hash tables on every word. Each bucket of a hash
   * table has a list of pointers to connectors. These nodes also store if the chosen connector is
   * shallow.
   *
   * <p>
   *
   * <p>As with normal pruning, we make alternate left.right and right.left passes. In the R.L pass,
   * when we're on a word w, we make use of all the left-pointing hash tables on the words to the
   * right of w. After the pruning on this word, we build the left-pointing hash table this word.
   * This guarantees idempotence of the pass -- after doing an L.R, doing another would change
   * nothing.
   *
   * <p>Each connector has an integer c_word field. This refers to the closest word that it could be
   * connected to. These are initially determined by how deep the connector is. For example, a
   * deepest connector can connect to the neighboring word, so its c_word field is w+1 (w-1 if this
   * is a left pointing connector). It's neighboring shallow connector has a c_word value of w+2,
   * etc.
   *
   * <p>The pruning process adjusts these c_word values as it goes along, accumulating information
   * about any way of linking this sentence. The pruning process stops only after no disjunct is
   * deleted and no c_word values change.
   *
   * <p>The difference between RUTHLESS and GENTLE power pruning is simply that GENTLE uses the
   * deletable region array, and RUTHLESS does not. So we can get the effect of these two different
   * methods simply by always unsuring that deletable[][] has been defined. With nothing deletable,
   * this is equivalent to RUTHLESS. --DS, 7/97
   */
  public int power_prune(int mode, ParseOptions opts) {
      ...
      for (w = word.size() - 1; w >= 0; w--) {
        for (d = word.get(w).d; d != null; d = d.next) {
          ...
          if (right_connector_list_update(d.right, w, w, true) >= word.size()) {
            for (c = d.right; c != null; c = c.next) c.word = GlobalBean.BAD_WORD;
            for (c = d.left; c != null; c = c.next) c.word = GlobalBean.BAD_WORD;
            N_deleted++;
            total_deleted++;
          }
        }
    ...
    if (opts.verbosity > 2) opts.out.println("" + power_cost + " power prune cost:");
  ...
  /**
   * This runs through all the connectors in this table, and eliminates those who are obsolete. The
   * word fields of an obsolete one has been set to BAD_WORD.
   */
  ...
  /**
   * take this connector list, and try to match it with the words w-1, w-2, w-3...Returns the word
   * to which the first connector of the list could possibly be matched. If c is null, returns w. If
   * there is no way to match this list, it returns a negative number. If it does find a way to
   * match it, it updates the c.word fields correctly.
   */
   ...
   * take this connector list, and try to match it with the words w+1, w+2, w+3...Returns the word
   * to which the first connector of the list could possibly be matched. If c is null, returns w. If
   * there is no way to match this list, it returns a number greater than N_words-1 If it does find
   * a way to match it, it updates the c.word fields correctly.
   */
  ...
  /**
   * this takes two connectors (and whether these are shallow or not) (and the two words that these
   * came from) and returns true if it is possible for these two to match based on local
   * considerations.
   */
  public boolean possible_connection(
      Connector lc, Connector rc, boolean lshallow, boolean rshallow, int lword, int rword) {
  ...
  */
    ...
    for (w = 0; w < word.size(); w++) {
      ...
      for (d = word.get(w).d; d != null; d = xd) {
        ...
        if ((set_dist_fields(d.left, w, -1) < 0)
            || (set_dist_fields(d.right, w, 1) >= word.size())) {
          d.next = null;
        } else {
          d.next = head;
          head = d;
   ...
   * same place. The answer must be masked to the appropriate table size.
   *
   * <p>This is exactly the same hash function used in fast-match.
   */
  ...
  */
  ...
  public static final int CMS_SIZE = (2 << 10);
  ...
  public boolean match_in_cms_table(String pp_match_name) {
    ...
    for (cms = cms_table[cms_hash(pp_match_name)]; cms != null; cms = cms.next) {
      if (Postprocessor.post_process_match(pp_match_name, cms.name)) return true;
    }
    return false;
  }
  ...
  public Cms lookup_in_cms_table(String str) {
    ...
    for (cms = cms_table[cms_hash(str)]; cms != null; cms = cms.next) {
      if (str.equals(cms.name)) return cms;
    }
    return null;
  }
    ...
    for (hashval = 0; hashval < ls.hash_table_size; hashval++) {
      for (p = ls.hash_table[hashval]; p != null; p = p.next) {
        ...
        for (s = 0; s < name.length() && Character.isUpperCase(name.charAt(s)); s++)
          ;
        for (; s < name.length(); s++) if (name.charAt(s) != '*') name.setCharAt(s, '#');
        t = 0;
        for (s = 0; s < name.length() && Character.isUpperCase(name.charAt(s)); s++) {
          t++;
        }
        ...
        for (; s < name.length() && bad == 0; s++, t++) {
          ...
          if (!match_in_cms_table(name.toString())) bad++;
          name.setCharAt(s, '#');
        }
