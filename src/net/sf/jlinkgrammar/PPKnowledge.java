...
/**
 * I also, unfortunately, want to propose a new type of domain. These would include everything that
 * can be reached from the root word of the link, to the right, that is closer than the right word
 * of the link. (They would not include the link itself.)
 *
 * <p>In the following sentence, then, the "Urfl_Only Domain" of the G link would include only the
 * "O" link:
 *
 * <pre>
 * +-----G----+
 ...
 * </PRE>
 *
 * In the following sentence it would include the "O", the "TT", the "I", the second "O", and the
 * "A".
 *
 * <PRE>
 * +----------------G---------------+
 ...
 *
 * <p>kicking dogs bores me
 *
 * <p>kicking dogs kicks dogs
 *
 * <p>explaining the program is easy
 *
 * <p>explaining the program is running
 *
 * <p>(These are distinctions that I thought we would never be able to make, so I told myself they
 * were semantic rather than syntactic. But with domains, they should be easy.)
 *
 * <p>Modifications
 *
 * <p>3) when postprocessing a sentence, the links of each domain are placed in a set for quick
 * lookup, ('contains one' and 'contains none')
 *
 * <p>7) observation: the 'contains one' is, empirically, by far the most violated rule, so it
 * should come first in applying the rules.
 */
public class PPKnowledge {
  ...
  /**
   * handles to sets of links specified in knowledge file. These constitute auxiliary data,
   * necessary to implement the rules, below. See comments in post-process.c for a description of
   * these.
   */
  ...
  public PPKnowledge(ParseOptions opts, String dictname, String path) throws IOException {
    ...
    if (f == null) throw new RuntimeException("pp_lexer_open: passed a null file");
    lt = new PPLexTable();
    Reader f1 = f;
    lt.yylex(this, f1);
  ...
  /**
   * This is a degenerate class of rules: either a single rule asserting connectivity is there, or
   * it isn't. The only information in the rule (besides its presence) is the error message to
   * display if the rule is violated
   */
  private void read_connected_rule(ParseOptions opts, String label) {
    ...
    if (!lt.pp_lexer_set_label(label)) {
      ...
      if (opts.verbosity > 0) opts.out.println("PP warning: Not using 'link is connected' rule");
      return;
    }
  ...
  private void read_form_a_cycle_rules(ParseOptions opts, String label) {
    ...
    if (!lt.pp_lexer_set_label(label)) {
      ...
      if (opts.verbosity > 0) opts.out.println("PP warning: Not using any 'form a cycle' rules");
    } else {
    ...
    for (r = 0; r < n_form_a_cycle_rules; r++) {
      ...
      if (n_tokens <= 0) throw new RuntimeException("syntax error in knowledge file");
      lsHandle = PPLinkset.PPLinkset_open(n_tokens);
      for (i = 0; i < n_tokens; i++) PPLinkset.PPLinkset_add(lsHandle, tokens[i]);
      ...
      if (n_tokens > 1)
        throw new RuntimeException(
            "post_process: Invalid syntax: rule " + (r + 1) + " of " + label);
  ...
  private void read_bounded_rules(ParseOptions opts, String label) {
    ...
    if (!lt.pp_lexer_set_label(label)) {
      n_bounded_rules = 0;
      if (opts.verbosity > 0) opts.out.println("PP warning: Not using any 'bounded' rules");
    } else {
      n_commas = lt.pp_lexer_count_commas_of_label();
  ...
  /**
   * Reading the 'contains_one_rules' and reading the 'contains_none_rules' into their respective
   * arrays
   */
  private PPRule[] read_contains_rules(ParseOptions opts, String label) {
    ...
    if (!lt.pp_lexer_set_label(label)) {
      nRules = 0;
      if (opts.verbosity > 0) opts.out.println("PP warning: Not using any " + label + " rules");
    } else {
      n_commas = lt.pp_lexer_count_commas_of_label();
      nRules = (n_commas + 1) / 3;
    }
    ...
    for (r = 0; r < nRules; r++) {
      ...
      if (n_tokens > 1)
        throw new RuntimeException(
            "post_process: Invalid syntax in " + label + " (rule " + (r + 1) + ")");
      rules[r] = new PPRule();
  ...
  private void initialize_set_of_links_starting_bounded_domain() {
    ...
    set_of_links_starting_bounded_domain =
        PPLinkset.PPLinkset_open(GlobalBean.PP_MAX_UNIQUE_LINK_NAMES);
      ...
      for (j = 0; (d = (starting_link_lookup_table[j].domain)) != -1; j++)
        if (d == domain_of_rule)
          PPLinkset.PPLinkset_add(
              set_of_links_starting_bounded_domain, starting_link_lookup_table[j].starting_link);
    }
  }

  /**
   * read link set, marked by label in knowledge file, into a set of links whose handle is returned.
   * Return null if link set not defined in file, in which case the set is taken to be empty.
   */
  private PPLinkset read_link_set(ParseOptions opts, String label) {
    ...
    if (!lt.pp_lexer_set_label(label)) {
      ...
      n_strings = 0;
    } else n_strings = lt.pp_lexer_count_tokens_of_label();
  ...
  /**
   * read table of [link, domain type]. This tells us what domain type each link belongs to. This
   * lookup table *must* be defined in the knowledge file.
   */
  private void read_starting_link_table() {
    ...
    if ((n_tokens % 2) != 0)
      throw new RuntimeException(
          "post_process: Link table must have format [<link> <domain name>]+");
