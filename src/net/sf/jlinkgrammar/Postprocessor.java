...
/**
 * O.K. is this a static class? No it is used in the dictionary instance object which is then copied
 * to the Sentence instance object, and then to the various Linkage instance objects. The
 * programmmer will have to pay special attention to acertain wich postprocessor is used on any
 * given linkage.
 */
public class Postprocessor {
  ...
  /**
   * string comparison in postprocessing. The first parameter is a post-processing symbol. The
   * second one is a connector name from a link. The upper case parts must match. We imagine that
   * the first arg is padded with an infinite sequence of "#" and that the 2nd one is padded with
   * "*". "#" matches anything, but "*" is just like an ordinary char for matching purposes. For
   * efficiency sake there are several different versions of these functions
   */
  ...
  static int internal_process(Postprocessor pp, Sublinkage sublinkage, String msg[]) {
    ...
    if (!apply_relevant_rules(
        ...
        msg)) {
      for (i = 0; i < pp.pp_data.length; i++) pp.pp_data.word_links[i] = null;
    ...
    if (!apply_relevant_rules(
        ...
        msg)) return 1;
    if (!apply_rules(
        pp, new ApplyMustFormACycle(), sublinkage, pp.knowledge.form_a_cycle_rules, msg)) return 1;
    if (!apply_rules(pp, new ApplyConnected(), sublinkage, pp.knowledge.connected_rules, msg))
      return 1;
    if (!apply_rules(pp, new ApplyBounded(), sublinkage, pp.knowledge.bounded_rules, msg)) return 1;
    return 0; /* This linkage satisfied all the rules */
  }

  /**
   * returns false if the string s does not match anything in the array. The array elements are
  ...
  static boolean string_in_list(String s, String a[]) {
    /*  */
    int i;
    for (i = 0; a[i] != null; i++) if (Postprocessor.post_process_match(a[i], s)) return true;
    return false;
  }
  ...
  /**
   * Returns true if the linkage is connected, considering words that have at least one edge....this
   * allows conjunctive sentences not to be thrown out.
   *
   * @param pp
  ...
  static boolean is_connected(Postprocessor pp) {
    ...
    int i;
    for (i = 0; i < pp.pp_data.length; i++) pp.visited[i] = (pp.pp_data.word_links[i] == null);
    mark_reachable_words(pp, 0);
    for (i = 0; i < pp.pp_data.length; i++) if (!pp.visited[i]) return false;
    return true;
  }
  ...
  /**
   * fill in the pp.pp_data.word_links array with a list of words neighboring each word (actually a
   * list of links). The dir fields are not set, since this (after fat-link-extraction) is an
   * undirected graph.
   *
   * <p>TODO - Why does this pass in a PostProcessor object? How can this.pp_data ever be filled out
   * if we don't copy it into this instance object?
   *
  ...
  /**
   * build the domains acociated with a sublinkage. TODO - Why does this pass in a PostProcessor
   * object? How can this.pp_data ever be filled out if we don't copy it into this instance object?
   *
   * @param pp
   * @param sublinkage
   */
  ...
  static void build_domains(Postprocessor pp, Sublinkage sublinkage) {
    ...
    for (link = 0; link < sublinkage.num_links; link++) {
      ...
      if (PPLinkset.PPLinkset_match(pp.knowledge.ignore_these_links, s)) continue;
      if (PPLinkset.PPLinkset_match(pp.knowledge.domain_starter_links, s)) {
        ...
        if (PPLinkset.PPLinkset_match(pp.knowledge.urfl_domain_starter_links, s)) {
          ...
          add_link_to_domain(pp, link);
          bad_depth_first_search(
              pp, sublinkage, sublinkage.link[link].r, sublinkage.link[link].l, link);
          pp.pp_data.N_domains++;
          if (!(pp.pp_data.N_domains < GlobalBean.PP_MAX_DOMAINS)) {
          ...
          left_depth_first_search(
              pp, sublinkage, sublinkage.link[link].l, sublinkage.link[link].r, link);
          pp.pp_data.N_domains++;
          if (!(pp.pp_data.N_domains < GlobalBean.PP_MAX_DOMAINS)) {
            throw new RuntimeException("raise PP_MAX_DOMAINS value");
          }
    ...
    for (d = 0; d < pp.pp_data.N_domains; d++) {
      ...
      if (i == -1)
        throw new RuntimeException(
            "\tpost_process: Need an entry for "
  ...
  static void build_domain_forest(Postprocessor pp, Sublinkage sublinkage) {
    int d, d1, link;
    DTreeLeaf dtl;
    if (pp.pp_data.N_domains > 0) pp.pp_data.domain_array[pp.pp_data.N_domains - 1].parent = null;
  ...
  static int find_domain_name(Postprocessor pp, String link) {
    ...
    for (i = 0; ; i++) {
      ...
      if (Postprocessor.post_process_match(sllt[i].starting_link, link)) return domain;
    }
  }
  ...
  static boolean contained_in(Domain d1, Domain d2, Sublinkage sublinkage) {
    ...
    for (lol = d2.lol; lol != null; lol = lol.next) mark[lol.link] = true;
    for (lol = d1.lol; lol != null; lol = lol.next) if (!mark[lol.link]) return false;
    return true;
  }
  ...
  static boolean link_in_domain(int link, Domain d) {
    ...
    ListOfLinks lol;
    for (lol = d.lol; lol != null; lol = lol.next) if (lol.link == link) return true;
    return false;
  }
  ...
  static void depth_first_search(
      Postprocessor pp, Sublinkage sublinkage, int w, int root, int start_link) {
    ListOfLinks lol;
    pp.visited[w] = true;
    for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
    ...
    for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
      if (!pp.visited[lol.word]
          ...
          && !(lol.word < root
              && lol.word < w
              && PPLinkset.PPLinkset_match(
                  pp.knowledge.restricted_links, sublinkage.link[lol.link].name)))
  ...
  static void d_depth_first_search(
      Postprocessor pp, Sublinkage sublinkage, int w, int root, int right, int start_link) {
    ListOfLinks lol;
    pp.visited[w] = true;
    for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
      if ((lol.word < w) && (lol.link != start_link) && (w != root)) {
   ...
   * @param start_link
   */
  static void left_depth_first_search(
      Postprocessor pp, Sublinkage sublinkage, int w, int right, int start_link) {
    ListOfLinks lol;
    pp.visited[w] = true;
    for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
      if (lol.word < w && lol.link != start_link) {
  ...
  /**
   * This is a depth first search of words reachable from w, excluding any direct edge between word
   * a and word b.
   *
   * @param pp
   * @param sublinkage
  ...
  static class ApplyContainsOne implements ApplyFn {
     ...
     * @param sublinkage
     * @param rule
     * @return returns true if and only if all groups containing the specified link contain at least
     *     one from the required list. (as determined by exact string matching)
     */
    public boolean applyfn(Postprocessor pp, Sublinkage sublinkage, PPRule rule) {
      ...
      for (d = 0; d < pp.pp_data.N_domains; d++) {
        for (dtl = pp.pp_data.domain_array[d].child;
            dtl != null
                && !Postprocessor.post_process_match(rule.selector, sublinkage.link[dtl.link].name);
            dtl = dtl.next)
          ;
        if (dtl != null) {
    ...
    public boolean applyfn(Postprocessor pp, Sublinkage sublinkage, PPRule rule) {
      ...
      for (d = 0; d < pp.pp_data.N_domains; d++) {
        ...
        if (dtl != null) {
          /* selector link of rule appears in this domain */
          for (dtl = pp.pp_data.domain_array[d].child; dtl != null; dtl = dtl.next)
            if (string_in_list(sublinkage.link[dtl.link].name, rule.link_array)) return false;
        }
      }
      return true;
    }
  ...
  static class ApplyContainsOneGlobally implements ApplyFn {
    ...
    public boolean applyfn(Postprocessor pp, Sublinkage sublinkage, PPRule rule) {
      ...
      for (i = 0; i < sublinkage.num_links; i++) {
        ...
        if (Postprocessor.post_process_match(rule.selector, sublinkage.link[i].name)) break;
      }
  ...
  */
    ...
    public boolean applyfn(Postprocessor pp, Sublinkage sublinkage, PPRule rule) {
      ...
      for (w = 0; w < pp.pp_data.length; w++) {
        for (lol = pp.pp_data.word_links[w]; lol != null; lol = lol.next) {
          ...
          if (!PPLinkset.PPLinkset_match(rule.link_set, sublinkage.link[lol.link].name)) continue;
          Arrays.fill(pp.visited, false);
  ...
  static boolean apply_rules(
      Postprocessor pp, ApplyFn applyfn, Sublinkage sublinkage, PPRule rule_array[], String msg[]) {
    int i;
    for (i = 0; (msg[0] = rule_array[i].msg) != null; i++)
      if (!applyfn.applyfn(pp, sublinkage, rule_array[i])) return false;
    return true;
  }
  ...
  static boolean apply_relevant_rules(
    ...
    for (i = 0; (idx = relevant_rules[i]) != -1; i++) {
      ...
      if (!applyfn.applyfn(pp, sublinkage, rule_array[idx])) return false;
    }
    return true;
  }
  ...
  static void alloc_pp_node(Postprocessor pp) {
    ...
    for (i = 0; i < GlobalBean.MAX_LINKS; i++) pp.pp_node.d_type_array[i] = null;
  }
   ...
   *
   * <p>TODO - Should this reference this.pp_node.d_type_array? - jlr
   *
   * @param pp
   */
