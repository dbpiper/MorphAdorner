...
/**
 * This class is meant to be a bean type interface to link grammar. All the options are preset and
 * specific actions have to be taken to override them.
 *
 * @author johnryan
 */
public class Parser {
  ...
  public static void InitializeVars(String arg[]) {
    ...
    for (; i < arg.length; i++) {
      if (arg[i].charAt(0) == '-') {
        if (arg[i].equals("-pp")) {
          if ((post_process_knowledge_file != null) || (i + 1 == arg.length)) print_usage(arg[0]);
          post_process_knowledge_file = arg[i + 1];
          i++;
        } else if (arg[i].equals("-c")) {
          if ((constituent_knowledge_file != null) || (i + 1 == arg.length)) print_usage(arg[0]);
          constituent_knowledge_file = arg[i + 1];
          i++;
        } else if (arg[i].equals("-a")) {
          if ((affix_file != null) || (i + 1 == arg.length)) print_usage(arg[0]);
          affix_file = arg[i + 1];
          i++;
          ...
          if ((opts.input != System.in) || (i + 1 == arg.length)) print_usage(arg[0]);
          try {
          ...
          if ((opts.out != System.out) || (i + 1 == arg.length)) print_usage(arg[0]);
          try {
    ...
    if (!pp_on && post_process_knowledge_file != null) print_usage(arg[0]);
    ...
    if (pp_on && post_process_knowledge_file == null) {
      ...
      // TODO: logging
      System.err.println(
          "No post process knowledge file specified.  Using " + post_process_knowledge_file + ".");
    }
    ...
    if (cons_on && constituent_knowledge_file == null) {
      ...
      // TODO: logging
      System.err.println(
          "No constituent knowledge file specified.  Using " + constituent_knowledge_file + ".");
    }
  ...
  public static void doIt(String arg[]) throws IOException {
    ...
    while (GlobalBean.fget_input_string(input_string, opts.input, opts.out, opts)) {
      ...
      if (num_linkages == 0 && !opts.parse_options_get_batch_mode()) {
        if (opts.verbosity > 0) opts.out.println("No complete linkages found.");
    ...
    if (opts.parse_options_get_batch_mode()) {
      ...
      opts.out.println(
          ""
              + GlobalBean.batch_errors
              + " error"
              + ((GlobalBean.batch_errors == 1) ? "" : "s")
              + ".");
    }
  }
