...
public class ParseInfo {
  ...
  int disjunct_cost() {
    ...
    lcost = 0;
    for (i = 0; i < N_words; i++) {
      if (chosen_disjuncts[i] != null) lcost += chosen_disjuncts[i].cost;
    }
    return lcost;
  }
  ...
  DISNode build_DIS_CON_tree() {
    ...
    */
    ...
    for (w = 0; w < N_words; w++) dfs_height[w] = 0;
    for (w = 0; w < N_words; w++) height_dfs(w, GlobalBean.MAX_SENTENCE);

    for (w = 0; w < N_words; w++) height_perm[w] = new Integer(w);
    Arrays.sort(
        height_perm,
        0,
        N_words,
    ...
    for (w = 0; w < N_words; w++) dfs_root_word[w] = -1;
      ...
      for (; m != null; m = m.next) {
        ...
        for (lcost = 0; lcost <= cost; lcost++) {
          ...
          if (Lmatch) {
            ...
            if (le.multi) ls[1] = parse_set(sent, ld, d, lw, w, le, d.left.next, lcost, opts);
            if (d.left.multi) ls[2] = parse_set(sent, ld, d, lw, w, le.next, d.left, lcost, opts);
          ...
          if (Rmatch) {
            ...
            if (d.right.multi) rs[1] = parse_set(sent, d, rd, w, rw, d.right, re.next, rcost, opts);
            if (re.multi) rs[2] = parse_set(sent, d, rd, w, rw, d.right.next, re, rcost, opts);
          ...
          for (i = 0; i < 4; i++) {
            ...
            for (j = 0; j < 4; j++) {
              ...
              a_choice =
                  new ParseChoice(ls[i], lw, w, le, d.left, rs[j], w, rw, d.right, re, ld, d, rd);
              xt.set.put_choice_in_set(a_choice);
            }
          }
  ...
  XTableConnector x_table_store(
      int lw, int rw, Connector le, Connector re, int cost, ParseSet set) {
  ...
  int x_hash(int lw, int rw, Connector le, Connector re, int cost) {
    ...
    i =
        i
            + (i << 1)
            + MyRandom.randtable[
                (((le == null ? 0 : le.hashCode()) + i) % (x_table_size + 1))
                    & (GlobalBean.RTSIZE - 1)];
    i =
        i
            + (i << 1)
            + MyRandom.randtable[
                (((re == null ? 0 : re.hashCode()) + i) % (x_table_size + 1))
                    & (GlobalBean.RTSIZE - 1)];
  ...
  static ListOfLinks word_links[] =
      new ListOfLinks[GlobalBean.MAX_SENTENCE]; /* ptr to l.o.l. out of word */
  static int dfs_root_word[] = new int[GlobalBean.MAX_SENTENCE]; /* for the depth-first search */
  static int dfs_height[] =
      new int[GlobalBean.MAX_SENTENCE]; /* to determine the order to do the root word dfs */
  static Integer height_perm[] =
      new Integer[GlobalBean.MAX_SENTENCE]; /* permute the vertices from highest to lowest */
}
