...
public class PPLexTable {
  ...
  public boolean pp_lexer_set_label(String label) {
    ...
    if (idx_of_active_label == -1) return false; /* label not found */
  ...
  private int get_index_of_label(String label) {
    int i;
    for (i = 0; labels[i] != null; i++) if (labels[i].equals(label)) return i;
    return -1;
  }
  ...
  public int pp_lexer_count_tokens_of_label() {
    ...
    int n;
    PPLabelNode p;
    if (idx_of_active_label == -1) throw new RuntimeException("pp_lexer: current label is invalid");
  ...
  /**
   * add the set of strings, defined earlier by label_of_set, to the set of strings associated with
   * the current label
   */
    ...
    if ((idx_of_label_of_set = get_index_of_label(label_of_set)) == -1)
      throw new RuntimeException(
          "pp_lexer: label "
              ...
              + yylineno
              + ")");
    for (p = nodes_of_label[idx_of_label_of_set]; p != null; p = p.next) add_string_to_label(p.str);
  }

  static void check_string(String str) {
    if (str.length() > 1 && str.indexOf(',') >= 0)
      throw new RuntimeException(
          "pp_lexer: string " + str + " contains a comma, which is a no-no.");
  }
  ...
  public String[] pp_lexer_get_next_group_of_tokens_of_label() {
    ...
    tokens = new String[n];
    p = current_node_of_active_label;
    for (n = 0; p != null && !p.str.equals(","); n++, p = p.next) tokens[n] = p.str;
  ...
  /**
   * Reads a knowledge object into a table for use by other methods. Called by PPKnowledge.
   *
   * @param ppknowledge
   * @param f
