...
/**
 * This is one of the core classes of the link grammar. It associates a dictionary with a sentence.
 * The First routine you have to call is. Dictionary(ParseOptions opts, String dict_name, String
 * pp_name, String cons_name, String affix_name)
 *
 * <p>The dictionary format:
 *
 * <p>In what follows: Every "%" symbol and everything after it is ignored on every line. Every
 * newline or tab is replaced by a space.
 *
 * <p>The dictionary file is a sequence of ENTRIES. Each ENTRY is one or more WORDS (a sequence of
 * upper or lower case letters) separated by spaces, followed by a ":", followed by an EXPRESSION
 * followed by a ";". An EXPRESSION is a lisp expression where the functions are "&" or "and" or "|"
 * or "or", and there are three types of parentheses: "()", "{}", and "[]". The terminal symbols of
 * this grammar are the connectors, which are strings of letters or numbers or *s. (This description
 * applies to the prefix form of the dictionary. the current dictionary is written in infix form. If
 * the defined constant INFIX_NOTATION is defined, then infix is used otherwise prefix is used.)
 *
 * <p>The connector begins with an optinal @, which is followed by an upper case sequence of
 * letters. Each subsequent *, lower case letter or number is a subscript. At the end is a + or -
 * sign. The "@" allows this connector to attach to one or more other connectors.
 *
 * <p>Here is a sample dictionary entry (in infix form):
 *
 * <p>gone: T- & {&#064EV+};
 *
 * <p>(See our paper for more about how to interpret the meaning of the dictionary expressions.)
 *
 * <p>A previously defined word (such as "gone" above) may be used instead of a connector to specify
 * the expression it was defined to be. Of course, in this case, it must uniquely specify a word in
 * the dictionary, and have been previously defined.
 *
 * <p>If a word is of the form "/foo", then the file current-dir/foo is a so-called word file, and
 * is read in as a list of words. A word file is just a list of words separted by blanks or
 * newlines.
 *
 * <p>A word that contains the character "_" defines an idiomatic use of the words separated by the
 * "_". For example "kind of" is an idiomatic expression, so a word "kind_of" is defined in the
 * dictionary. Idomatic expressions of any number of words can be defined in this way. When the word
 * "kind" is encountered, all the idiomatic uses of the word are considered.
 *
 * <p>An expresion enclosed in "[..]" is give a cost of 1. This means that if any of the connectors
 * inside the square braces are used, a cost of 1 is incurred. (This cost is the first element of
 * the cost vector printed when a sentence is parsed.) Of course if something is inside of 10 levels
 * of "[..]" then using it incurs a cost of 10. These costs are called "disjunct costs". The
 * linkages are printed out in order of non-increasing disjunct cost.
 *
 * <p>The expression "(A+ or ())" means that you can choose either "A+" or the empty expression
 * "()", that is, that the connector "A+" is optional. This is more compactly expressed as "{A+}".
 * In other words, curly braces indicate an optional expression.
 *
 * <p>The expression "(A+ or [])" is the same as that above, but there is a cost of 1 incurred for
 * choosing not to use "A+". The expression "(EXP1 & [EXP2])" is exactly the same as "[EXP1 &
 * EXP2]". The difference between "({[A+]} & B+)" and "([{A+}] & B+)" is that the latter always
 * incurrs a cost of 1, while the former only gets a cost of 1 if "A+" is used.
 *
 * <p>The dictionary writer is not allowed to use connectors that begin in "ID". This is reserved
 * for the connectors automatically generated for idioms.
 *
 * <p>One more thing...
 *
 * <p><B>The Dictionary is a binary tree</b>
 *
 * <p>The data structure storing the dictionary is simply a binary tree. There is one catch however.
 * The ordering of the words is not exactly the order given by strcmp. It was necessary to modify
 * the order to make it so that "make" < "make.n" < "make-up" The problem is that if some other
 * string could lie between '\0' and '.' (such as '-' which strcmp would give) then it makes it much
 * harder to return all the strings that match a given word. For example, if "make-up" was inserted,
 * then "make" was inserted the a search was done for "make.n", the obvious algorithm would not find
 * the match. <CODE>
...
public class Dictionary {
  ...
  /**
   * We link together all the Exp structs that are allocated in reading this dictionary. Used for
   ...
   * paramater called "path". If this is non-null, then the path used to find the file is taken from
   * that path. Otherwise the path is taken from the dict_name. This is only needed because an
   * affix_file is opened by a recursive call to this function.
   *
   * @param opts the set of parse options is copied into the Sentence object and Linkage Object as
   *     well as the GlobalBean Object TODO - design the ParseOptions record keeping for better
  ...
  /**
   * This is the dictionary constructor method. It is the equivilent of the C function
   * create_dictionary It calls a private constructor of the same name.
   */
  public Dictionary(
      ParseOptions opts, String dict_name, String pp_name, String cons_name, String affix_name)
      throws IOException {
    this(opts, dict_name, pp_name, cons_name, affix_name, null);
  }

  /**
   * read rules from path and initialize the appropriate fields in a postprocessor structure, a
   * pointer to which is returned. The only reason we need the dictname is to used it for the path,
   * in case there is no DICTPATH set up. If the dictname is null, and there is no DICTPATH, it just
   * uses the filename as the full path.
   *
   * <p>
   *
   ...
   */
  Postprocessor post_process_open(ParseOptions opts, String dictname, String path)
      throws IOException {
  ...
  /**
   * Opens the dictionary, sets the path and assigns the Dictionary object's filepointer to the
   * dictionary specified in ParseOptions.
   *
  ...
  /**
   * Read the dictionary into memory. This musr be preceded by opening the dictionary and seting the
   * file pointer using open_dictionary.
   *
  ...
  /**
   * Returns a pointer to a lookup list of the words in the dictionary. This list is made up of
   * DictNodes, linked by their right pointers. The node, file and string fields are copied from the
   * dictionary.
   *
   * <p>Freeing this list elsewhere is unnecessary, as long as the rest of the program merely
   * examines the list (doesn't change it)
   */
  ...
  */
  ...
  /**
   * assuming that s is a pointer to a dictionary string, and that t is a pointer to a search
   * string, this returns 0 if they match, >0 if s>t, and <0 if s<t.
   *
   * <p>The matching is done as follows. Walk down the strings until you come to the end of one of
   * them, or until you find unequal characters. A "*" matches anything. Otherwise, replace "." by
   * "\0", and take the difference. This behavior matches that of the function dict_compare().
   */
  int dict_match(String s, String t) {
    ...
    if (i < s.length() && i < t.length() && (s.charAt(i) == '*' || t.charAt(i) == '*')) return 0;
    return ((i >= s.length() || s.charAt(i) == '.') ? 0 : s.charAt(i))
        - ((i >= t.length() || t.charAt(i) == '.') ? 0 : t.charAt(i));
  }

  /**
   * We need to prune out the lists thus generated. A sub string will only be considered a subscript
   * if it followes the last "." in the word, and it does not begin with a digit.
   */
  boolean true_dict_match(String s, String t) {
    ...
    if ((dt >= 0) && (dt == t.length() - 1 || Character.isDigit(t.charAt(dt + 1)))) dt = -1;
    if ((ds >= 0) && (ds == s.length() - 1 || Character.isDigit(s.charAt(ds + 1)))) ds = -1;

    if (dt < 0 && ds >= 0) {
      if (t.length() > ds) return false; /* we need to do this to ensure that */
      return (s.startsWith(t)); /*"i.e." does not match "i.e" */
    } else if (dt >= 0 && ds < 0) {
      if (s.length() > dt) return false;
   ...
   *
   * <p>correct such string:
   *
   * <p>() contains no "."
   *
   * <p>() non-empty strings separated by _
   *
   * <p>
   *
  ...
  /**
   * This might be a good place for entity extraction since all cap words often represent entities
   * US, DOD etc. Perhaps we should add a dictionary?
   *
   * @param word
  ...
  static boolean is_initials_word(String word) {
    ...
    */
    ...
    for (i = 0; i < word.length() - 1; i += 2) {
      ...
      if (word.charAt(i + 1) != '.') return false;
    }
    return true;
  }
  ...
  /**
   * returns true iff it's an appropriately formed hyphenated word. This means all letters, numbers,
   * or hyphens, not beginning and ending with a hyphen.
   *
   * @param s
  ...
  static boolean is_ly_word(String s) {
    ...
    int i = s.length();
    if (i < 4) return false;
    if (s.endsWith("ly")) return true;
    return false;
  }

  // TODO: return boolean
  /**
   * if the string contains a single ".", and ends in ".Ix" where x is a number, return x. Return -1
   * if not of this form.
   */
  ...
  /**
   * This gets the next character from the input, eliminating comments. If we're in quote mode, it
   * does not consider the % character for comments
   */
   ...
   * This has the effect of creating what used to be called an optional node.
   */
  ...
  */
  ...
  /**
   * The data structure storing the dictionary is simply a binary tree. There is one catch however.
   * The ordering of the words is not exactly the order given by strcmp. It was necessary to modify
   * the order to make it so that "make" < "make.n" < "make-up" The problem is that if some other
   * string could lie between '\0' and '.' (such as '-' which strcmp would give) then it makes it
   * much harder to return all the strings that match a given word. For example, if "make-up" was
   * inserted, then "make" was inserted the a search was done for "make.n", the obvious algorithm
   * would not find the match.
   *
   * <p>verbose version
   *
   * <PRE>
  ...
  /**
   * Insert the new node into the dictionary below node n give error message if the new element's
   * string is already there assumes that the "n" field of new is already set, and the left and
   * right fields of it are null
   */
  ...
  /**
   * Insert the list into the dictionary. It does the middle one first, then the left half, then the
   * right.
   *
      ...
      if (token.charAt(0) == '/') {
        ...
        if (dn == null) {
          throw new RuntimeException(
              "Error reading word file " + token.toString() + ", file empty?");
        }
      } else {
        dn_new = new DictNode();
        dn_new.left = dn;
  ...
  /**
   * Takes as input a pointer to a DictNode. The string of this DictNode is an idiom string. This
   * string is torn apart, and its components are inserted into the dictionary as special idiom
   * words (ending in .I*, where * is a number). The expression of this DictNode (its node field)
   * has already been read and constructed. This will be used to construct the special idiom
   * expressions. The given dict node is freed. The string is also freed.
   */
  ...
  /**
   * This function is used to open a dictionary file or a word file, or any associated data file
   * (like a post process knowledge file).
   *
   * <p>It works as follows. If the file name begins with a "/", then it's assumed to be an absolute
   * file name and it tries to open that exact file.
   *
   * <p>If the filename does not begin with a "/", then it uses the dictpath mechanism to find the
   * right file to open. This looks for the file in a sequence of directories until it finds it. The
   * sequence of directories is specified in a dictpath string, in which each directory is followed
   * by a ":".
   *
   * <p>The dictpath that it uses is constructed as follows. If the dictname is non-null, and is an
   * absolute path name (beginning with a "/", then the part after the last "/" is removed and this
   * is the first directory on the dictpath. After this comes the DICTPATH environment variable,
  ...
  /**
   * Allocates string space and returns a pointer to it. In this string is placed the idiomized name
   * of the given string s. This is the same as s, but with a postfix of ".Ix", where x is an
   * appropriate number. x is the minimum number that distinguishes this word from others in the
   * dictionary.
   */
  ...
  /**
   * Tear the idiom string apart. Destroys the string s, but does not free it. Put the parts into a
   * list of DictNodes (connected by their right pointers) Sets the string fields of these DictNodes
   * pointing to the fragments of the string s. Later these will be replaced by correct names (with
   * .Ix suffixes). The list is reversed from the way they occur in the string. A pointer to this
   * list is returned.
   */
