...
/**
 * The linkage routines are primarily used to walk linkages and retrieve information about the
 * sentence. You must fist call dictionary_create, sentence,_create, sentence_parse, in that order.
 * Then you can process links.
 *
...
public class Linkage {
  ...
  /**
   * Generate the list of all links of the indexth parsing of the sentence. For this to work, you
   * must have already called parse, and already built the whole_set.
   */
   ...
   *
   * @param sent sentence to extract links from. TODO - does not set the current Linkage objects
   *     this.sent to sent. This may be a bug!
   * @param opts options to use when processing TODO - does not set the current Linkage objects
   *     this.opts to opts. This may be a bug!
   */
  ...
  /**
   * The old C comment said -- This procedure mimics analyze_fat_linkage in order to extract the
   * sublinkages and copy them to the Linkage data structure passed in. BUT no linkage structure is
   * passed in the Linkage structure is the instance variable of this Linkage.
   *
   * @param sent sentence to extract links from. TODO - does not set the current Linkage objects
   *     this.sent to sent. This may be a bug!
   * @param opts options to use when processing TODO - does not set the current Linkage objects
  ...
  /**
   * This is a toplevel routine used to setup a for loop to process each sublinkage (usually an AND
   * disjunct).
   *
  ...
  /**
   * This is need to set up the for loop to walk down a set of links for printing or other
   * processing returns the size of the linkage array as the number of links
   *
  ...
  /**
   * A method used to find the current number of linkages. To create a new linkage you will have to
   * add one to the result of this method It uses a looop to find the next available linkage index
   *
  ...
  /**
   * setter function to set the current sublinkage to indicated index TODO - does this function have
   * to insure a compact array? If so should it return the next available index?
   *
  ...
  boolean link_already_appears(Link link, int a) {
    ...
    for (i = 0; i < a; ++i) {
      for (j = 0; j < sublinkage[i].num_links; ++j) {
        if (links_are_equal(sublinkage[i].link[j], link)) return true;
      }
    }
    return false;
  }
  ...
  Sublinkage unionize_linkage() {
    ...
    for (i = 0; i < num_sublinkages; ++i) {
      for (j = 0; j < sublinkage[i].num_links; ++j) {
        ...
        if (!link_already_appears(link, i)) num_in_union++;
      }
    }
  ...
  public String linkage_get_link_label(int index) {
    Link link;
    if (!verify_link_index(index)) return null;
  ...
  public int linkage_get_link_rword(int index) {
    Link link;
    if (!verify_link_index(index)) return -1;
    link = sublinkage[current].link[index];
    return link.r;
  }
  ...
  public String linkage_print_links_and_domains() {
    ...
    for (link = 0; link < N_links; link++) {
      if (linkage_get_link_lword(link) == -1) continue;
  ...
  public String build_linkage_postscript_string() {
    ...
    if (dict.left_wall_defined) {
      ...
      if (!opts.display_walls)
        for (j = 0; j < N_links; j++) {
          if (ppla[j].l == 0) {
            if (ppla[j].r == num_words - 1) continue;
            N_wall_connectors++;
      ...
      print_word_0 =
          (((!suppressor_used) && (N_wall_connectors != 0))
              || (N_wall_connectors > 1)
              || opts.display_walls);
    } else {
      print_word_0 = true;
    ...
    for (link = 0; link < N_links; link++) {
      ...
      if (!print_word_N && (ppla[link].r == num_words - 1)) continue;
      if (ppla[link].l == -1) continue;
      if ((j % 7 == 0) && (j > 0)) string.append("\n");
      j++;
      string.append("[");
    ...
    for (j = 0; j < N_rows; j++) {
      ...
      } else string.append(row_starts[j]);
    }
    string.append("]\n");
  ...
  public String linkage_print_diagram() {
    ...
    for (link_length = 1; link_length < N_words_to_print; link_length++) {
      for (j = 0; j < N_links; j++) {
        ...
        if ((ppla[j].r - ppla[j].l) != link_length) continue;
        ...
        if (!print_word_N && (ppla[j].r == num_words - 1)) continue;
        ...
        if (opts.display_link_subscripts) {
          if (!Character.isLetter(s.charAt(0))) s = "";
        } else {
        ...
        connector = s;
        k = 0;
        if (opts.display_link_subscripts) k = connector.length();
        else
        ...
        if (opts.display_link_subscripts) {
          ...
          while (sss < s.length()
              && Character.isUpperCase(s.charAt(sss))
              && picture[row][ttt] == '-') {
            picture[row][ttt] = s.charAt(sss);
            ttt++;
            sss++;
   ...
   *
   * @param mode mode 1: treebank-style constituent tree mode 2: flat, bracketed tree [A like [B
   *     this B] A] mode 3: flat, treebank-style tree (A like (B this) )
   * @return Returns the constituent tree in one of three modes, treebank, flat bracketed, or flat
   *     treebank
   */
   ...
   *
   * <p>The other useful method for dealing with a cosntituent tree is:
   *
   * <p>String linkage_print_constituent_tree(int mode);
   *
   * @return A CNode is a standard tree data structure. The children of a node are stored as a
   *     linked list, with the end of the list indicated by next==NULL. The start and end fields of
   *     a node indicate the span of the constituent, with the first word indexed by 0. Leaves are
   *     defined by the condition child==NULL.
   */
  ...
  private void count_words_used() {
    ...
    if (unionized && num_subl > 1) num_subl--;

    if (opts.verbosity >= 2) opts.out.println("Number of sublinkages = " + num_subl);
    for (i = 0; i < num_subl; i++) {
      for (w = 0; w < num_words; w++) word_used[i][w] = 0;
      current = i;
      ...
      if (opts.verbosity >= 2) {
        ...
        for (w = 0; w < num_words; w++) {
          if (word_used[i][w] == 0) opts.out.print("0 ");
          if (word_used[i][w] == 1) opts.out.print("1 ");
        }
        opts.out.println();
      }
    }
  ...
  private String print_flat_constituents() {
    ...
    if (num_subl > GlobalBean.MAXSUBL) {
      ...
      if (opts.verbosity >= 2)
        opts.out.println(
            "Number of sublinkages exceeds maximum: only considering first "
                + GlobalBean.MAXSUBL
                + " sublinkages");
    }
    if (unionized && num_subl > 1) num_subl--;
  ...
  private String exprint_constituent_structure(int numcon_total) {
    ...
    if (opts.verbosity >= 2) opts.out.println();
  ...
  private int gen_comp(
      int numcon_total, int numcon_subl, String ctype1, String ctype2, String ctype3, int x) {
    ...
    for (c1 = numcon_total; c1 < numcon_total + numcon_subl; c1++) {
      ...
      if ((x == 2) && (constituent[c1].domain_type == 't')) continue;

      /* If it's domain-type z, it's a subject-relative Clause;
      the VP doesn't need an NP */
      if (constituent[c1].domain_type == 'z') continue;
      ...
      if ((x == 1 || x == 2)
          && ((!Postprocessor.post_process_match("S", constituent[c1].start_link)
              ...
              || Postprocessor.post_process_match("S##w", constituent[c1].start_link))) continue;
      ...
      if ((x == 3)
          && (!Postprocessor.post_process_match("Rn", constituent[c1].start_link)
              ...
              && !Postprocessor.post_process_match("MX#d", constituent[c1].start_link))) continue;

      /* If ctype1 is SBAR (Clause opener case), it has to be an f domain */
      if ((x == 5) && (constituent[c1].domain_type != 'f')) continue;

      /* If ctype1 is SBAR (pp opener case), it has to be a g domain */
      if ((x == 6) && (constituent[c1].domain_type != 'g')) continue;

      /* If ctype1 is NP (paraphrase case), it has to be started by an SI */
      if ((x == 7) && !Postprocessor.post_process_match("SI", constituent[c1].start_link)) continue;

      /* If ctype1 is VP (participle modifier case), it has to be
      started by an Mv or Mg */
      if ((x == 8) && !Postprocessor.post_process_match("M", constituent[c1].start_link)) continue;
      ...
      if (!(constituent[c1].type.equals(ctype1))) continue;
        ...
        for (w3 = constituent[c1].right; w3 < num_words; w3++) {
          for (c2 = numcon_total; (done == 0) && (c2 < numcon_total + numcon_subl); c2++) {
            if (!((constituent[c2].left == w2) && (constituent[c2].right == w3)) || (c2 == c1))
              continue;
            if (!(constituent[c2].type.equals(ctype2))) continue;
            ...
            if ((x == 5) || (x == 6) || (x == 9)) {
              ...
              while (true) {
                if (word_used[current][w] == 1) break;
                w++;
              }
        ...
        for (w2 = constituent[c].left - 1; (done == 0) && w2 >= 0; w2--) {
          for (c2 = numcon_total; c2 < numcon_total + numcon_subl; c2++) {
            ...
            if ((constituent[c2].domain_type == 'v') || (constituent[c2].domain_type == 'a')) {
              ...
              if (opts.verbosity >= 2) opts.out.println("Adjusting constituent " + c2 + ":");
              print_constituent(c2);
            }
          }
        }
        if (word.get(constituent[c].left).equals(",")) constituent[c].left++;
      }
    }
  }
  ...
  private void adjust_for_right_comma(int c) {
    ...
    if ((word.get(constituent[c].right).equals(","))
        || (word.get(constituent[c].right).equals("RIGHT-WALL"))) {
      w--;
      while (true) {
        if (word_used[current][w] == 1) break;
        w--;
  ...
  private void print_tree(StringBuffer cs, boolean indent, CNode n, int o1, int o2) {
    ...
    if (indent) for (i = 0; i < o1; ++i) cs.append(" ");
    cs.append("(");
    cs.append(n.label);
    cs.append(" ");
    ...
    for (m = n.child; m != null; m = m.next) {
      if (m.child == null) {
        cs.append(m.label);
        if ((m.next != null) && (m.next.child == null)) cs.append(" ");
      } else {
        if (m != n.child) {
          if (indent) cs.append("\n");
          else cs.append(" ");
        ...
        if ((m.next != null) && (m.next.child == null)) {
          if (indent) {
            cs.append("\n");
            for (i = 0; i < child_offset; ++i) cs.append(" ");
          } else cs.append(" ");
        }
      }
    }
  ...
  private int read_constituents_from_domains(int numcon_total, int s) {
    ...
    for (d = 0, c = numcon_total; d < subl.pp_data.N_domains; d++, c++) {
      ...
      for (dlink = domain.lol; dlink != null; dlink = dlink.next) {
        ...
        if (linkage_get_link_rword(l) > rightmost) rightmost = linkage_get_link_rword(l);
      }
      ...
      if (Postprocessor.post_process_match("CP", constituent[c].start_link)) {
        if (word.get(leftmost).equals(",")) constituent[c].left++;
      ...
      if (Postprocessor.post_process_match("QI", constituent[c].start_link)
        ...
        w = leftmost;
        if (word.get(w).equals(",")) w++;
        if (wordtype[w] == GlobalBean.WType_NONE) name = "WHADVP";
        else if (wordtype[w] == GlobalBean.WType_QTYPE) name = "WHNP";
        else if (wordtype[w] == GlobalBean.WType_QDTYPE) name = "WHNP";
        else if (true) {
        ...
        if (wordtype[w] == GlobalBean.WType_QDTYPE) {
          ...
          for (w2 = w + 1; w2 < r_limit - 1; w2++)
            if ((wordtype[w2] == GlobalBean.WType_STYPE)
                || (wordtype[w2] == GlobalBean.WType_PTYPE)) break;
        ...
        for (c2 = numcon_total; c2 < numcon_total + numcon_subl; c2++) {
          if ((constituent[c].left < constituent[c2].left)
            ...
            if ((word.get(constituent[c2].right).equals(","))
              ...
              if (opts.verbosity >= 2) {
                opts.out.println(
                    "WARNING: the constituents aren't nested! Adjusting them.("
                        + c
                        + ", "
                        + c2
                        + ")");
    ...
    for (c = numcon_total; c < numcon_total + numcon_subl; c++) {
      ...
      if (((constituent[c].domain_type == 'v')
              && (wordtype[linkage_get_link_rword(constituent[c].start_num)]
                  == GlobalBean.WType_PTYPE))
  ...
  private int find_next_element(int start, int numcon_total, int num_elements, int num_lists) {
    ...
    for (c = start + 1; c < numcon_total; c++) {
      ...
      if (!constituent[templist[0]].type.equals(constituent[c].type)) continue;
      ok = 1;
      ...
      for (a = 0; a < num_elements; a++) {
        if (constituent[c].subl == constituent[templist[a]].subl) ok = 0;
        if (((constituent[c].left < constituent[templist[a]].left)
            ...
            || ((constituent[c].left > constituent[templist[a]].left)
                && (constituent[c].right < constituent[templist[a]].right))) ok = 0;
        for (c2 = 0; c2 < numcon_total; c2++) {
          if (constituent[c2].canon != constituent[c].canon) continue;
          for (c3 = 0; c3 < numcon_total; c3++) {
            if ((constituent[c3].canon == constituent[templist[a]].canon)
                && (constituent[c3].subl == constituent[c2].subl)) ok = 0;
          }
        }
      }
      if (ok == 0) continue;
  ...
  private void generate_misc_word_info() {
    ...
    for (w1 = 0; w1 < num_words; w1++) wordtype[w1] = GlobalBean.WType_NONE;
    ...
    for (l1 = 0; l1 < linkage_get_num_links(); l1++) {
      ...
      if (Postprocessor.post_process_match("Mr", label1)) wordtype[w1] = GlobalBean.WType_QDTYPE;
      if (Postprocessor.post_process_match("MX#d", label1)) wordtype[w1] = GlobalBean.WType_QDTYPE;
    }
  }
  ...
  private int last_minute_fixes(int numcon_total) {
    ...
    int c,
        c2,
        global_leftend_found,
        adjustment_made,
        global_rightend_found,
        lastword,
        newcon_total = 0;
    ...
    for (c = 0; c < numcon_total; c++) {
      ...
      if (uppercompare(constituent[c].start_link, "YS")
          || uppercompare(constituent[c].start_link, "YP")) {
        constituent[c].right++;
      }
    ...
    for (c = 0; c < numcon_total; c++) {
      if ((constituent[c].right == (num_words) - 3)
          ...
          && (sent.word.get((num_words) - 2).string.equals("."))) constituent[c].right++;
    }
    ...
    for (c = 0; c < numcon_total; c++) {
      if ((constituent[c].left == 1)
          && (constituent[c].type.equals("S"))
          && (constituent[c].valid == 1)) global_leftend_found = 1;
    }
    for (c = 0; c < numcon_total; c++) {
      if ((constituent[c].right >= lastword)
          && (constituent[c].type.equals("S"))
          && (constituent[c].valid == 1)) global_rightend_found = 1;
    }
    if ((global_leftend_found == 0) || (global_rightend_found == 0)) {
      ...
      constituent[c].domain_type = 'x';
      numcon_total++;
      if (opts.verbosity >= 2) opts.out.println("Adding global sentence constituent:");
      print_constituent(c);
    }
  ...
  private int merge_constituents(int numcon_total) {
      ...
      for (c2 = 0; c2 < numcon_total; c2++) {
        ...
        for (c3 = 0; c3 < numcon_total; c3++) {
          if ((constituent[c2].canon == constituent[c3].canon)
              && (constituent[c3].subl == constituent[c1].subl)) ok = 0;
        }
        for (c3 = 0; c3 < numcon_total; c3++) {
    ...
    for (c1 = 0; c1 < numcon_total; c1++) {
      if (constituent[c1].valid == 0) continue;
      for (c2 = c1 + 1; c2 < numcon_total; c2++) {
        if (constituent[c2].canon == constituent[c1].canon) constituent[c2].valid = 0;
      }
    }
    ...
    for (n = 0; n < num_lists; n++) {
      ...
      for (n2 = 0; n2 < num_lists; n2++) {
        if (n2 == n) continue;
        if (andlist[n2].num < andlist[n].num) continue;
        listmatch = 1;
        for (a = 0; a < andlist[n].num; a++) {
          match = 0;
          for (a2 = 0; a2 < andlist[n2].num; a2++) {
            if (andlist[n2].e[a2] == andlist[n].e[a]) match = 1;
          }
          if (match == 0) listmatch = 0;
          /* At least one element was not matched by n2 */
        }
        if (listmatch == 1) andlist[n].valid = false;
      }
    }
          ...
          for (a2 = 0; (a2 < andlist[n2].num) && (andlist[n].valid); a2++) {
            ...
            for (a3 = 0; a3 < andlist[n].num; a3++) {
              ...
              if ((constituent[c2].left > constituent[c3].left)
                  || (constituent[c2].right < constituent[c3].right)) ok = 0;
            }
            if (ok != 0) continue;
    ...
    for (n = 0; n < num_lists; n++) {
      ...
      if (opts.verbosity >= 2) opts.out.println("Adding constituent:");
   ...
   *
   * @param mode Print out the constituent tree. mode 1: treebank-style constituent tree mode 2:
   *     flat, bracketed tree [A like [B this B] A] mode 3: flat, treebank-style tree (A like (B
   *     this) )
