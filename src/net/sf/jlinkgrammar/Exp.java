...
/**
 * TODO add javadoc The E_list an Exp structures defined below comprise the expression trees that
 * are stored in the dictionary. The expression has a type (AND, OR or TERMINAL). If it is not a
 * terminal it has a list (an EList) of children.
 */
public class Exp {
  ...
  int size_of_expression() {
    ...
    int size;
    ExpList el;
    if (type == GlobalBean.CONNECTOR_type) return 1;
    size = 0;
  ...
  void insert_connectors(int dir) {
    ...
    /*    dummy.my_word = NO_WORD; */
    /* turn off the length part of the matching */
