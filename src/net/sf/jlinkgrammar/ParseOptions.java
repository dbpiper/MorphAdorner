...
/**
 * ParseOptions keeps a set of state variables regarding user choices. A parse option is associated
 * with each sentence and each linkage. It is important to keep track of which parse option instance
 * object is being referenced in any given situation.
 *
 * <p>Here's how the default values are initialized
 *
 * <ul>
 *   <li>verbosity = 5;
...
public class ParseOptions {
   ...
   *
   * @see Sentence#sentence_parse(ParseOptions) default 0
   */
  ...
  /**
   * If true, then linkages with islands (separate component of the link graph) will be generated
   * (default=false)
   *
   * @see Sentence#count(int, int, Connector, Connector, int, ParseOptions)
   */
  ...
  public void parse_options_set_display_constituents(ParseOptions opts, int dummy) {
    if ((dummy < 0) || (dummy > 2)) {
      ...
      display_constituents = 0;
    } else display_constituents = dummy;
  }
  ...
  Switch default_switches[] = {
    ...
    new Switch(
        "null-block",
        false,
        "Size of blocks with null cost true",
        new Setter() {
          public void set(int value) {
            null_block = value;
          }
  ...
  public void print_expression(Exp n) {
    ...
    if (n.type == GlobalBean.CONNECTOR_type) {
      for (i = 0; i < n.cost; i++) out.print("[");
      out.print(n.string + n.dir);
      for (i = 0; i < n.cost; i++) out.print("]");
    } else {
      for (i = 0; i < n.cost; i++) out.print("[");
      if (n.cost == 0) out.print("(");
      for (el = n.l; el != null; el = el.next) {
        print_expression(el.e);
        if (el.next != null) {
          if (n.type == GlobalBean.AND_type) out.print(" & ");
          if (n.type == GlobalBean.OR_type) out.print(" | ");
        }
      }
      for (i = 0; i < n.cost; i++) out.print("]");
      if (n.cost == 0) out.print(")");
